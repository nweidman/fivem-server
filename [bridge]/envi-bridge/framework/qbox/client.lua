...
Framework.SetPlayerData = function(PlayerData)
    ...
    if PlayerData.money then
        ...
        for account, amount in pairs(PlayerData.money) do
            Store.Set(('account.%s'):format(account), amount)
...
Framework.IsPlayerDead = function()
    if GetResourceState('qbx_medical') ~= 'missing' then
...
AddStateBagChangeHandler('hunger', ('player:%s'):format(Store.ServerId), function(_, _, value)
    Store.Set(('status.%s'):format('hunger'), value)
end)

AddStateBagChangeHandler('thirst', ('player:%s'):format(Store.ServerId), function(_, _, value)
    Store.Set(('status.%s'):format('thirst'), value)
end)

AddStateBagChangeHandler('stress', ('player:%s'):format(Store.ServerId), function(_, _, value)
    Store.Set(('status.%s'):format('stress'), value)
end)