...
Framework.SetPlayerData = function(PlayerData)
    ...
    if PlayerData.metadata then
        if PlayerData.metadata['hunger'] then
            Store.Set(('status.%s'):format('hunger'), PlayerData.metadata['hunger'])
        end
        if PlayerData.metadata['thirst'] then
            Store.Set(('status.%s'):format('thirst'), PlayerData.metadata['thirst'])
        end
        if PlayerData.metadata['stress'] then
            Store.Set(('status.%s'):format('stress'), PlayerData.metadata['stress'])
        end
    end
