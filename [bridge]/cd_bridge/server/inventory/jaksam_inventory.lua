...
function RemoveItem(source, item_name, amount)
    return exports.jaksam_inventory:removeItem(source, item_name, amount)
end
...
function GetSlotFromFirstItemFound(source, item_name)
    local item, slot = exports.jaksam_inventory:getItemByName(source, item_name)
    if not item and not slot then return nil end
    return slot
end
