...
local moneyFunc = {
    -- QBOX
    {   framework = QBXExport,
        chargePlayer = function(src, amount, moneyType)
            local ply = exports.qbx_core:GetPlayer(src)
            if not ply then return false end
            return ply.Functions.RemoveMoney(moneyType or "cash", amount)
        end,
        fundPlayer = function(src, amount, moneyType)
            local ply = exports.qbx_core:GetPlayer(src)
            if not ply then return false end
            return ply.Functions.AddMoney(moneyType or "cash", amount)
        end,
    },
    ...
    {   framework = ESXExport,
        chargePlayer = function(src, amount, moneyType)
            ...
            if (moneyType or "cash") == "bank" then
        ...
        serverSide =
            function(src)
                ...
                Player = {
                    ...
                    cash = info.Functions.GetMoney("cash"),
