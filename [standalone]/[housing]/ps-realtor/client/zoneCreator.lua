...
function zoneCreator.adjustHeight(delta)
    if #zoneCreator.polygon == 0 then return end

    -- Move base points down/up
    for i = 1, #zoneCreator.polygon do
        local p = zoneCreator.polygon[i]
        zoneCreator.polygon[i] = vec3(p.x, p.y, p.z - delta)
    end

    -- Expand/shrink equally upward and downward
    zoneCreator.height = math.max(0.5, zoneCreator.height + (delta * 2))

    -- Update extremes
    zoneCreator.lowestPoint = vec3(
        zoneCreator.lowestPoint.x,
        zoneCreator.lowestPoint.y,
        zoneCreator.lowestPoint.z - delta
    )

    zoneCreator.highestPoint = vec3(
        zoneCreator.highestPoint.x,
        zoneCreator.highestPoint.y,
        zoneCreator.highestPoint.z - delta
    )
    ...
    while true do
        ...
        if hit then
            ...
            if IsDisabledControlJustPressed(0, 38) and entity > 0 and GetEntityType(entity) == 3 then
                ...
                local input = lib.inputDialog('Enter Kind of Door', {
                    { type = 'input', label = 'single/gates/garage', icon = 'keyboard' }
                })
                local text = ''
                if input then
                    text = input[1]
                ...
                if text ~= 'single' and text ~= 'gates' and text ~= 'garage' then text = 'single' end
