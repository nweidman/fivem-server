...
local function CheckCamMovementInput(cam)
    ...
    if camPos ~= currentCamPos then
        ...
        local distance = #(coords - currentCamPos)
        if distance < maxDistance then
            SetCamCoord(cam, camPos)
            gPedPosition = camPos
        else
            local _coord = vec3(coords.x, coords.y, coords.z + 1.5)
...
local function PlaceLastSelectedFurniture(position, rotation)
    ...
    if furniture then
        ...
        ClearPreviewObject()
        Client.Functions.TriggerServerCallback(_e("Server:PlaceFurniture"), function(response)
            if response.error then
                return Client.Functions.SendNotify(locale("game.error", response.error), "error")
            end
            Client.Functions.SendNotify(locale("lFurniture.furniture_placed"), "success")
        end, furniture, Client.Player.inApartment, Client.Player.inRoom.room_id)
    end
end
...
local function isPointInRectangle(point, rect)
    -- local x, y = point.x, point.y
    -- local rectX = { rect[1].x, rect[2].x, rect[3].x, rect[4].x }
    -- local rectY = { rect[1].y, rect[2].y, rect[3].y, rect[4].y }

    -- return x >= math.min(rectX[1], rectX[3]) and x <= math.max(rectX[1], rectX[3]) and
    --     y >= math.min(rectY[1], rectY[3]) and y <= math.max(rectY[1], rectY[3])
    return true
end

local function GetRoomStashCount()
...
local function GetRoomWeedPlanterCount()
    local furnitures = Client.Player.inRoom.furnitures
    local count = 0
    for key, furniture in pairs(furnitures) do
        local model = furniture.model
            if isModelWeedPlanter(model) and furniture.isPlaced then
            count += 1
        end
    end
    return count
...
local function PreviewFurniture(furniture, place, buy, category)
    ...
    if pcall(lib.requestModel, modelHash, 500) then
        if isModelStash(modelHash) then
            local max_stash_count = Config.StashOptions.max_stash_count
            if GetRoomStashCount() >= max_stash_count then
                Client.Functions.SendNotify(locale("game.cant_place_more_stash"), "error")
                return false
            end
        end
        if isModelWeedPlanter(modelHash) then
            if GetRoomWeedPlanterCount() >= 1 then
                Client.Functions.SendNotify(locale("game.cant_place"), "error")
                return false
            end
        end
        ...
        if previewObject ~= 0 then
            ...
            CreateThread(function()
                ...
                if Client.Player.inApartment == -1 then
                    local roomId = Client.Player.inRoom.room_id ---@type number
                    if not isPointInRectangle(state.position, Config.MloRooms[roomId].poly) then
                        Client.Functions.SendNotify(locale("game.wrong_place"), "error")
                        return ClearPreviewObject()
                    end
                end
                -- @ --
                if place then
                    ...
                    if DoesEntityExist(previewObject) then
                        if buy then
                            Client.Functions.TriggerServerCallback(_e("Server:BuyFurniture"), function(response)
                                if response.error then
...
local function OwnedFurniturePutInStorage(furniture)
    Client.Functions.TriggerServerCallback(_e("Server:OwnedFurniturePutInStorage"), function(response)
        if response.error then
            Client.Functions.SendNotify(locale("game.error", response.error), "error")
        else
...
RegisterNUICallback("nui:previewFurniture", function(data, resultCallback)
    ...
    resultCallback(true)
    local objectId = PreviewFurniture(furniture, false, true, category)
    if objectId then
...
RegisterNUICallback("nui:ownedFurnitureReSell", function(furniture, resultCallback)
    Client.Functions.TriggerServerCallback(_e("Server:OwnedFurnitureReSell"), function(response)
        if response.error then
            Client.Functions.SendNotify(locale("game.error", response.error), "error")
