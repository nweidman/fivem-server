...
local PropertyDoors = {} -- [doorId] = locked

function dpDiscord(playerid)
    local discord = false
...
function GetHighestDonorLevel(source)
    if dpDiscord(source) then
        local roles = exports.Badger_Discord_API:GetDiscordRoles(source)
        local rolesAccess = {}
        for i = 1, #Config.Roles, 1 do
            for j = 1, #roles do
                if exports.Badger_Discord_API:CheckEqual(Config.Roles[i], roles[j]) then
                    table.insert(rolesAccess, Config.Roles[i])
                end
            end
        end
        local roleTables = {}
        for i = 1, #Config.Roles, 1 do
            for j = 1, #rolesAccess do
                if Config.Roles[i] == rolesAccess[j] then
                    table.insert(roleTables, i)
                end
            end
        end
        local hr = #roleTables
        local hr2 = roleTables[hr]
        return Config.Roles[hr2]
    else
        return 'none'
    end
end
...
function Property:new(propertyData)
    ...
    Framework[Config.Inventory].RegisterInventory(stashName, 'Property: ' ..  (propertyData.street or propertyData.apartment or 'Unknown') .. ' #'.. propertyData.property_id or propertyData.apartment or stashName, stashConfig)
    --     end
    -- end
...
function Property:UpdateDescription(data)
    ...
    local oldDesc = self.propertyData.description
    ...
    Framework[Config.Logs].SendLog("**PROPERTY DESCRIPTION CHANGED**\n\n**Property ID: **" .. self.property_id .. "\n**Realtor: **" .. GetPlayerName(realtorSrc) .. "\n**Old Description: **" .. tostring(oldDesc).. "\n**New Description: **" .. description, 'orange')
...
function Property:UpdatePrice(data)
    ...
    local realtorSrc = data.realtorSrc
    local oldPrice = self.propertyData.price
    ...
    Framework[Config.Logs].SendLog("**PROPERTY PRICE CHANGE**\n\n**Property ID: **" .. self.property_id .. "\n**Realtor:** " .. GetPlayerName(realtorSrc).. "\n**Old Price: **" .. tostring(oldPrice) .. "\n**New Price: **" .. price, 'orange')
...
function Property:UpdateForSale(data)
    ...
    Framework[Config.Logs].SendLog("**CHANGED FOR SALE**\n\n**Property ID:** " .. self.property_id .. "\n**Realtor:** " .. GetPlayerName(realtorSrc).. "\n**For-Sale Value: **" .. tostring(forsale), 'orange')
...
function Property:UpdateShell(data)
    ...
    local realtorSrc = data.realtorSrc
    local oldShell = self.propertyData.shell
    ...
    Framework[Config.Logs].SendLog("**PROPERTY SHELL CHANGE**\n\n**Property ID: **" .. self.property_id .. "\n**Realtor: **" .. GetPlayerName(realtorSrc).. "\n**Old Shell: **" .. tostring(oldShell).. "\n**New Shell: **" .. shell, 'orange')
...
function Property:UpdateOwner(data)
    ...
    local houses = MySQL.query.await('SELECT shell FROM properties WHERE owner_citizenid = ?', {citizenid})
    local houseCount = 0
    ...
    if houseCount >= Config.MaxHouses then 
        Framework[Config.Notify].Notify(targetSrc, "You can not own more than 3 properties", "error")
        Framework[Config.Notify].Notify(realtorSrc, "Client already owns 3 properties", "error")
        return 
    end
    ...
    local supporter = GetHighestDonorLevel(targetSrc)
    if not Config.DiscountRoles[supporter] or supporter == 'none' then
        ...
        local price = self.propertyData.price - (Config.DiscountRoles[supporter] * self.propertyData.price)
        targetPlayer.Functions.RemoveMoney('bank', price, "Bought Property: " .. self.propertyData.street .. " " .. self.property_id)
    end
    ...
    local totalAfterCommission = self.propertyData.price * 0.5
    ...
    if self.propertyData.owner ~= nil then
        transfer = true
    end
    ...
    MySQL.update("UPDATE cd_doorlock SET perms = @perms WHERE location_group = @location_group", {
        ["@perms"] = json.encode({
            job = {},
            ace = {},
        ...
        ["@location_group"] = "PROPERTY_"..tostring(self.property_id)
    })

    self.propertyData.furnitures = {} -- to be fetched on enter

    TriggerClientEvent("ps-housing:client:updateProperty", -1, "UpdateOwner", self.property_id, citizenid, transfer)
    TriggerClientEvent("ps-housing:client:updateProperty", -1, "UpdateForSale", self.property_id, 0)
    
    Framework[Config.Logs].SendLog("**PROPERTY PURCHASED**\n\n**Property ID:** " ..self.property_id.."\n**Purchased by: **"..PlayerData.charinfo.firstname.." "..PlayerData.charinfo.lastname.."\n**Price: **$"..self.propertyData.price.."\n**Realtor: **"..realtor.PlayerData.charinfo.firstname.." "..realtor.PlayerData.charinfo.lastname.."", 'green')
...
function Property:UpdateImgs(data)
    ...
    Framework[Config.Logs].SendLog("**PROPERTY IMAGES CHANGE**\n\n**Property ID:** " .. self.property_id .. "\n**Realtor:** " .. GetPlayerName(realtorSrc), 'orange')
...
function Property:UpdateDoor(data)
    ...
    Framework[Config.Logs].SendLog("**PROPERTY DOOR CHANGE**\n\n**Property ID:** " .. self.property_id .. "\n**Realtor:** " .. GetPlayerName(realtorSrc), 'orange')
...
function Property:UpdateHas_access(data)
    ...
    if self.propertyData.shell == 'mlo' then
        local accessPlusOwner = table.clone(has_access)
        accessPlusOwner[#accessPlusOwner+1] = self.propertyData.owner
        MySQL.update("UPDATE cd_doorlock SET perms = @perms WHERE location_group = @location_group", {
            ["@perms"] = json.encode({
                job = {},
                ace = {},
                identifier = accessPlusOwner,
                items = {},
                discord = {}
            }),
            ["@location_group"] = "PROPERTY_"..tostring(self.property_id)
...
function Property:UpdateGarage(data)
    ...
    Framework[Config.Logs].SendLog("**PROPERTY GARAGE CHANGE**\n\n**Property ID:** " .. self.property_id .. "\n**Realtor:** " .. GetPlayerName(realtorSrc), 'orange')
...
function Property:UpdateApartment(data)
    ...
    local realtorSrc = data.realtorSrc
    local targetSrc = data.targetSrc
    local oldApartment = self.propertyData.apartment
    ...
    Framework[Config.Logs].SendLog("**APARTMENT CHANGE**\n\n**Property id:** " .. self.property_id .. "\n**Realtor: **" .. GetPlayerName(realtorSrc) .. "\n**Player: **" .. GetPlayerName(targetSrc) .. "\n**Old Apartment: **" .. tostring(oldApartment).. "\n**New Apartment: **" .. apartment, 'orange')
...
function Property:DeleteProperty(data)
    ...
    MySQL.Async.execute("DELETE FROM cd_doorlock WHERE location_group = @property_id", {
        ["@property_id"] = "PROPERTY_"..propertyid
    }, function (rowsChanged)
        if rowsChanged > 0 then
            Debug("Deleted doors for property with id: " .. propertyid, "by: " .. realtorName)
        end
    end)

    MySQL.Async.execute("DELETE FROM cd_doorlock_locationgroups WHERE name = @property_id", {
        ["@property_id"] = "PROPERTY_"..propertyid
    }, function (rowsChanged)
    ...
    Framework[Config.Logs].SendLog("**PROPERTY DELETED**\n\n**Property ID: **" .. propertyid .. "\n**Realtor:** " .. realtorName, 'red')
...
lib.callback.register('ps-housing:cb:getPlayersInProperty', function(source, property_id)
    ...
    for src, _ in pairs(property.playersInGarden) do
        local targetSrc = tonumber(src)
        if GetPlayerName(targetSrc) then
            if targetSrc ~= source and not property.playersInside[targetSrc] then
                local name = GetCharName(targetSrc)
            ...
            property.playersInGarden[targetSrc] = nil
        end
    end
...
RegisterNetEvent("ps-housing:server:buyFurniture", function(property_id, items, price, isGarden)
    ...
    Framework[Config.Logs].SendLog("**FURNITURE PURCHASE**\n\n**Player:** ".. GetPlayerName(src) .. "\n**Price: **$" .. price, 'black')
...
RegisterNetEvent("ps-housing:server:openQBInv", function(data)
    ...
    local citizenid = GetCitizenid(src)
    if isAllowedToRaid or property:CheckForAccess(citizenid) then 
...
lib.addCommand('furnish', {
    help = 'Opens Furniture Menu',
}, function(source)
    local src = source
    local Player = exports.qbx_core:GetPlayer(src)
    local insideMeta = Player.PlayerData.metadata["inside"]
    if not insideMeta or not insideMeta.property_id then return end
    TriggerClientEvent('ps-housing:client:openFurnitureMenu', src, insideMeta.property_id.."F")
end)
...
RegisterNetEvent("ps-housing:server:removeAccessNewOwner", function(property_id, citizenidToRemove)
    local src = source
