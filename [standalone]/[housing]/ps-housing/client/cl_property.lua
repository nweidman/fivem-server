...
local LoadedDoors = {}
local DoorStates = {} -- [doorId] = true/false (locked)
...
function Property:CreateShell()
    ...
    coords = vec3(coords.x, coords.y, coords.z + 1500.0)
...
function Property:RegisterMlo()
    ...
    center = getCenter(zoneData.points)
    ...
    if self.owner or accessAndConfig then
        ...
        exports['crm-radialmenu']:crm_add(
            {
                ...
                crm_items = {
                    {
                        crm_id = self.property_id.."F",
                        crm_title = "Furniture Menu",
                        crm_icon = "house",
                        crm_close_menu = true,
                        crm_action = {crm_type = "crm-client", crm_event = "ps-housing:client:openFurnitureMenu"},
                    },
                },
            })
        ...
        if self.owner then
            exports['crm-radialmenu']:crm_add(
            {
                ...
                crm_items = {
                    {
                        crm_id = self.property_id,
                        crm_title = "Manage Property",
                        crm_icon = "key",
                        crm_close_menu = true,
                        crm_action = {crm_type = "crm-client", crm_event = "ps-housing:client:openManagePropertyAccessMenu"},
                    }
                },
            })
        end
...
function Property:RemoveMenus(force)
    ...
    exports['crm-radialmenu']:crm_remove("crm-house")
    if self.owner then
...
function Property:ManageAccessMenu()
    if not self.inProperty and not self.inGarden then return end
...
function Property:UnloadFurnitures()
    ...
    for i = 1, #self.furnitureObjs do
        ...
        if furniture and furniture.propertyId == self.property_id then
...
RegisterNetEvent("ps-housing:client:updateProperty", function(type, property_id, data, transfer)
    local property = Property.Get(property_id)

    if not property then return end
    local PlayerData = exports.qbx_core:GetPlayerData()
    if type == "UpdateOwner" and transfer then
        TriggerServerEvent("ps-housing:server:removeAccessNewOwner", property_id, data)
        property:RemoveBlip()
    end
...
RegisterNetEvent("ps-housing:client:openFurnitureMenu", function(data)
    Wait(300)
    Modeler:OpenMenu(string.sub(data, 1, -2))
end)
...
AddEventHandler("ps-housing:client:openManagePropertyAccessMenu", function(data)
    local property = Property.Get(data)
    if not property then return end
