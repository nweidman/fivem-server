...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(ped) or Action.useWhileDead then
        if not isDoingAction then
            ...
            SendTextMessage('You are already doing something!')
        end
    else
        SendTextMessage('Cant do that action!')
    end
end
...
function DisableActions(ped)
    ...
    if Action.controlDisables.disableMovement and Action.label ~= 'Hitting The Blunt..' then
...
AddEventHandler('onResourceStop', function(name)
    ...
    print("The inventory or progressbar was restarted and the inventory was vacated")
    LocalPlayer.state:set("inv_busy", false, true)
end)