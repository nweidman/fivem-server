...
function submitForm(e) {
  $("#keypad").css("display", "none");
  $.post(
    "https://shanks-storagelockers/UseCombination",
    JSON.stringify({
      type: type,
      combination: e.value,
    })
...
$(function () {
  ...
  window.addEventListener("message", function (event) {
    ...
    if (eventData.action == "kekw") {
      $("body").css(
        "background-image",
        "url('https://i.kym-cdn.com/entries/icons/original/000/031/051/cover4.jpg')"
      );
  ...
  $("body").on("keyup", function (e) {
    if (CurrentType == "keypad") {
      ...
      if (e.keyCode == 48) {
        addKeyPadNumber({ value: "0" });
      } else if (e.keyCode == 49) {
        addKeyPadNumber({ value: "1" });
      } else if (e.keyCode == 50) {
        addKeyPadNumber({ value: "2" });
      } else if (e.keyCode == 51) {
        addKeyPadNumber({ value: "3" });
      } else if (e.keyCode == 52) {
        addKeyPadNumber({ value: "4" });
      } else if (e.keyCode == 53) {
        addKeyPadNumber({ value: "5" });
      } else if (e.keyCode == 54) {
        addKeyPadNumber({ value: "6" });
      } else if (e.keyCode == 55) {
        addKeyPadNumber({ value: "7" });
      } else if (e.keyCode == 56) {
        addKeyPadNumber({ value: "8" });
      } else if (e.keyCode == 57) {
        addKeyPadNumber({ value: "9" });
      } else if (e.keyCode == 8) {
        clearForm();
