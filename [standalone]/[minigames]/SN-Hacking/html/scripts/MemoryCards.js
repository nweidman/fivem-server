...
var memorycard_difficultys = {
  ...
  medium: {grid:[6,4], time: 45000},
  hard: {grid:[6,4], time: 50000},
};
...
async function StartMemoryCardCountDown() {
  ...
  while(i < memorycard_grid_size[0] * memorycard_grid_size[1]) {
    ...
    if (!memorycard_icons_using[randomIcon] || memorycard_icons_using[randomIcon] && memorycard_icons_using[randomIcon] == 1) {
      NewHtml+= `<div id="memorycard_${i}" class="ThermiteBox" onclick="MemoryCardClick(this)" data-icon=${randomIcon}><i class="fa-solid fa-question" style="font-size: 3vh;  position: static;"></i></div>`;
...
function MemoryCardClick(elem) {
  if (memorycard_running) {
    ...
    if (memorycard_selected1) {
      elem.innerHTML = `<i class="fa-solid fa-${elem.getAttribute('data-icon')}" style="font-size: 3vh; position: static;"></i>`;
      if (elem.getAttribute('data-icon') == memorycard_selected1.getAttribute('data-icon')) {
        ...
        setTimeout(function() {
          ...
          elem.innerHTML = '<i class="fa-solid fa-question" style="font-size: 3vh; position: static;"></i>';
          elem.style.pointerEvents = '';
          memorycard_selected1.style.backgroundColor = '';
          memorycard_selected1.innerHTML = '<i class="fa-solid fa-question" style="font-size: 3vh; position: static;"></i>';
