    ...
    QBCore.Commands.Add('revive', 'Revive a player', { { name = 'id', help = 'Player ID' } }, false,
        function(source, args)
            ...
            if args[1] then
                ...
                TriggerEvent('wasabi_ambulance:logRevive', src, nil)
            end
        end, 'admin')
...
RegisterNetEvent('wasabi:usebagofcoke', function()
    local src = source
    exports.ox_inventory:RemoveItem(src, 'purebagofcoke', 1)
end)

RegisterNetEvent('wasabi:usebagofweed', function()
    local src = source
    exports.ox_inventory:RemoveItem(src, 'purebagofweed', 1)
end)

RegisterNetEvent('wasabi:usebagofmeth', function()
    local src = source
    exports.ox_inventory:RemoveItem(src, 'purebagofmeth', 1)
end)

RegisterNetEvent('wasabi:usepainkillers', function()
    local src = source
    exports.ox_inventory:RemoveItem(src, 'painkillers', 1)
end)

RegisterNetEvent('wasabi:usebandage', function()
    local src = source
    exports.ox_inventory:RemoveItem(src, 'bandage', 1)
end)

RegisterNetEvent('wasabi:registerStash', function()
    local src = source
    local Player = exports.qbx_core:GetPlayer(src)
    if not Player then return end

    -- only for ambulance job
    ...
    exports.ox_inventory:RegisterStash(
        ...
        100,        -- slots
        1000000,    -- weight
        citizenid,  -- owner tied to player
        { ['ambulance'] = 0 } -- only ambulance job can access
    )
end)