...
RegisterNetEvent('wasabi_bridge:playerLoaded', function()
    ...
    for _, model in ipairs(Config.BigDogs) do
            ...
            if next(PlayerInjury) then
                ...
                if not OnPainKillers then
                    for k, v in pairs(PlayerInjury) do
                        ...
                        -- Limb handling
                        if not OnPainKillers and shouldLimp and GetPedMovementClipset(ped) ~= `move_m@injured` then
...
RegisterNetEvent('wasabi_ambulance:healIfaks', function(full, quiet)
    local ped = wsb.cache.ped
    local maxHealth = 200
    local health = GetEntityHealth(ped)
    local newHealth = math.min(maxHealth, health + 50)
    SetEntityHealth(ped, newHealth + 0.0)
    for _, injury in pairs(PlayerInjury) do
        if injury.data.bleed > 0 then
            injury.data.bleed = 0
...
RegisterNetEvent('wasabi_ambulance:healAfaks', function(full, quiet)
    local ped = wsb.cache.ped
    local maxHealth = 200
    local health = GetEntityHealth(ped)
    local newHealth = math.min(maxHealth, health + 80)
    SetEntityHealth(ped, newHealth + 0.0)
    for _, injury in pairs(PlayerInjury) do
        if injury.data.bleed > 0 then
            injury.data.bleed = 0
        end
        -- Remove limping/staggering effects
        if injury.data.limp then
            injury.data.limp = false
        end
    end
    ClearPatientSymptoms()
    -- Reset movement clipset to normal
    --ResetPedMovementClipset(ped, 0)
...
RegisterNetEvent('wasabi_ambulance:sedate', function()
    ...
    disableKeys = true
    if isDog then
        TaskPlayAnim(ped, 'creatures@rottweiler@move', 'dead_left', 8.0, 8.0, -1, 33, 0, false, false, false)
    else
...
RegisterNetEvent('wasabi_ambulance:useifaks')
AddEventHandler('wasabi_ambulance:useifaks', function()
    useIfaks()
end)

RegisterNetEvent('wasabi_ambulance:useafaks')
AddEventHandler('wasabi_ambulance:useafaks', function()
    useAfaks()
end)
...
RegisterCommand('help', function()
    if EMSAvailable < 2 then
        if not exports['envi-medic']:IsOnCooldown() and (isDead=='dead') and not isReviving and not IsCheckedIn and not (earlySpawnTimer > 0) then

            exports['envi-medic']:SendHelp()
            TriggerServerEvent('wasabi_ambulance:logHelp', GetEntityCoords(PlayerPedId()))
            
        else
            TriggerEvent('wasabi_bridge:notify', 'Not Dead', "You can not do this yet.", 'error')
        end
    else
        TriggerEvent('wasabi_bridge:notify', 'EMS on Duty', "There are EMS on duty, try /911ems.", 'error')
    end
end, false)