...
local bypassJobs = { -- These jobs do not count towards the max job limit
    auction = true,
    dj = true,
    -- casino = true,
    realestate = true,
    ...
    if citizenid then
        ...
        if playerData and (playerData.license == license2 or playerData.license == license) then
            ...
            local logMsg = ('**PLAYER**\n**Name:** %s\n**ID: **%s\n**Character Name: **%s\n**Citizen ID: **%s\n**License:** %s'):format(
                ...
                TriggerEvent('qb-log:server:CreateLog', 'joinleave', 'Player Loaded', 'green', logMsg)
                ...
                local logsTable = {
                    ...
                    ["joinleave"] = "join", -- This could be a Lua variable in your script
                    ["reason"] = "Joined the Server",
                    ["location"] = "<"..string.format("%.2f", playerData.position.x)..", "..string.format("%.2f", playerData.position.y)..", "..string.format("%.2f", playerData.position.z)..">", -- This could be a Lua variable in your script
                }
		        exports["fortislogs"]:addLog("joinleavelog", logsTable)
...
local function CountJobs(citizenid)
    local count = 0
    local player = GetPlayerByCitizenId(citizenid) or GetOfflinePlayer(citizenid)
    for jobName, _ in pairs(player.PlayerData.jobs) do
        if not bypassJobs[jobName] then
            count = count + 1
        end
    end
    return count
...
function AddPlayerToJob(citizenid, jobName, grade)
    ...
    -- if not IsPlayerAceAllowed(source, "admin") then
        if foodJobs[jobName] then
            for k, v in pairs(foodJobs) do
                if jobs[k] and k ~= jobName then
                    if not player.Offline then
                        exports.qbx_core:Notify(player.PlayerData.source, 'You already have a job at a restaurant. You can have at most 1 at a time.', 'error')
                    end
                    return false
                end
            end
        end
        if weedJobs[jobName] then
            for k, v in pairs(weedJobs) do
                if jobs[k] and k ~= jobName then
                    if not player.Offline then
                        exports.qbx_core:Notify(player.PlayerData.source, 'You already have a job at a weed store. You can have at most 1 at a time.', 'error')
                    end
                    return false
                end
            end
        end
        if mechJobs[jobName] then
            for k, v in pairs(mechJobs) do
                if jobs[k] and k ~= jobName then
                    if not player.Offline then
                        exports.qbx_core:Notify(player.PlayerData.source, 'You already have a job at a mechanic shop. You can have at most 1 at a time.', 'error')
                    end
                    return false
                end
            end
        end
        if dealerships[jobName] then
            for k, v in pairs(dealerships) do
                if jobs[k] and k ~= jobName then
                    if not player.Offline then
                        exports.qbx_core:Notify(player.PlayerData.source, 'You already have a job at a dealership. You can have at most 1 at a time.', 'error')
                    end
                    return false
                end
            end
    ...
    if CountJobs(citizenid) >= maxJobsPerPlayer
    -- and not IsPlayerAceAllowed(source, "admin")
    and not player.PlayerData.jobs[jobName]
    and not bypassJobs[jobName] then
        return false, {
            code = 'max_jobs',
...
function CheckPlayerData(source, playerData)
    ...
    -- Cuff Streak
    playerData.metadata.cuffstreak = tonumber(playerData.metadata.cuffstreak) or 0
    -- Fitbit
    ...
    -- Skills
    playerData.metadata.skills = playerData.metadata.skills or {}
    playerData.metadata.skills.level = tonumber(playerData.metadata.skills.level) or 1
    playerData.metadata.skills.xp = tonumber(playerData.metadata.skills.xp) or 100
    playerData.metadata.skills.perkpoints = tonumber(playerData.metadata.skills.perkpoints) or 0
    playerData.metadata.skills.gymcooldown = tonumber(playerData.metadata.skills.gymcooldown) or 0
    -- Money Wash Rep
    playerData.metadata.moneywashrep = tonumber(playerData.metadata.moneywashrep) or 0
    ...
    if not player.Offline then
        ...
        local logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name: **%s\n**Citizen ID: **%s\n\n**MONEY**\n**Account: **%s\n**Amount: **%s\n**New Balance: **%s\n**Reason **%s'):format(
            ...
            if amount > 100000 then
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen', logMsg, true)
                local logsTable = {
                    ...
                    ["type"] = moneyType, -- This could be a Lua variable in your script
                ...
                exports["fortislogs"]:addLog("moneylog", logsTable)
            else
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen', logMsg)
                local logsTable = {
                    ["steamname"] = player.PlayerData.name, -- This could be a Lua variable in your script
                    ["citizenid"] = player.PlayerData.citizenid, -- This could be a Lua variable in your script
                    ["ign"] = player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, -- This could be a Lua variable in your script
    ...
    if not player.Offline then
        ...
        if amount > 100000 then
                ...
                local logsTable = {
                    ...
                    ["type"] = moneyType, -- This could be a Lua variable in your script
                    ["amount"] = amount, -- This could be a Lua variable in your script
                    ["action"] = "removed", -- You can also just enter raw strings
                    ["reason"] = reason -- This could be a Lua variable in your script
                }
                exports["fortislogs"]:addLog("moneylog", logsTable)
            else
