...
lib.addCommand('callsign', {
    ...
    local player = exports.qbx_core:GetPlayer(source)
    player.Functions.SetMetaData('callsign', tostring(args.callsign))
end)
...
lib.addCommand('checkleo', {
    help = 'Check how many LEOs are clocked in',
    restricted = false
}, function(source)
    ...
    for _, playerId in pairs(Players) do
        ...
        if (Player.PlayerData.job.name == 'police' or Player.PlayerData.job.name == 'sheriff') and Player.PlayerData.job.onduty then -- Adjust job names accordingly
            leoCount = leoCount + 1
        end
    end

    local desc
    if leoCount >= THRESHOLD then
        desc = ('There are currently %d or more LEOs clocked in.'):format(THRESHOLD)
    else
        desc = ('There are currently %d LEO%s clocked in.'):format(leoCount, leoCount == 1 and '' or 's')
    end
