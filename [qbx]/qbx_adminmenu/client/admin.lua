...
local function toggleNoclip()
    CreateThread(function()
        ...
        AttachCamToEntity(noclipCam, ent,  0.0, -3.0, 1.0, true)
        ...
        while noclipEnabled do
            ...
            if IsDisabledControlPressed(2, 22) or IsDisabledControlPressed(2, 52) then -- Space or Q
                local setPos = GetOffsetFromEntityInWorldCoords(ent, 0.0, 0.0, multiplier * speed / 2)
                SetEntityCoordsNoOffset(ent, setPos.x, setPos.y, setPos.z, false, false, false)
                if not inVehicle then
                    SetEntityCoordsNoOffset(cache.ped, setPos.x, setPos.y, setPos.z, false, false, false)
                end
            elseif IsDisabledControlPressed(2, 51) then -- E
                local setPos = GetOffsetFromEntityInWorldCoords(ent, 0.0, 0.0, multiplier * -speed / 2)
                SetEntityCoordsNoOffset(ent, setPos.x, setPos.y, setPos.z, false, false, false)
                if not inVehicle then
                    SetEntityCoordsNoOffset(cache.ped, setPos.x, setPos.y, setPos.z, false, false, false)
...
local options = {
    ...
    function() TriggerEvent('wasabi_ambulance:revive') end,
    -- function()
...
RegisterNetEvent('qbx_admin:client:blips', function()
    if not showBlips then
        ...
        local logMsg = ('**Admin:** %s\n**Character Name:** %s\n**Citizen ID:** %s\n**Admin License:** %s'):format(Player.name, Player.charinfo.firstname .. " " .. Player.charinfo.lastname, Player.citizenid, Player.license)
        TriggerServerEvent('qb-log:server:CreateLog', 'admin_blips', '📌 Blips On', 'green', logMsg, false)
        ...
        local Player = exports.qbx_core:GetPlayerData()
        local logMsg = ('**Admin:** %s\n**Character Name:** %s\n**Citizen ID:** %s\n**Admin License:** %s'):format(Player.name, Player.charinfo.firstname .. " " .. Player.charinfo.lastname, Player.citizenid, Player.license)
        TriggerServerEvent('qb-log:server:CreateLog', 'admin_blips', '📌 Blips Off', 'red', logMsg, false)
...
RegisterNetEvent('qbx_admin:client:names', function()
    if not showNames then
        ...
        local Player = exports.qbx_core:GetPlayerData()
        local logMsg = ('**Admin:** %s\n**Character Name:** %s\n**Citizen ID:** %s\n**Admin License:** %s'):format(Player.name, Player.charinfo.firstname .. " " .. Player.charinfo.lastname, Player.citizenid, Player.license)
        TriggerServerEvent('qb-log:server:CreateLog', 'admin_names', '🙏 Names On', 'green', logMsg, false)
        ...
        local Player = exports.qbx_core:GetPlayerData()
        local logMsg = ('**Admin:** %s\n**Character Name:** %s\n**Citizen ID:** %s\n**Admin License:** %s'):format(Player.name, Player.charinfo.firstname .. " " .. Player.charinfo.lastname, Player.citizenid, Player.license)
        TriggerServerEvent('qb-log:server:CreateLog', 'admin_names', '🙏 Names Off', 'red', logMsg, false)   
...
CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/kill', 'Kill yourself or a target player.', {
        { name = 'id', help = '(optional) target server id' }
    })
    TriggerEvent('chat:addSuggestion', '/revive', 'Revive yourself or a target player.', {
        { name = 'id', help = '(optional) target server id' }
    })
end)
