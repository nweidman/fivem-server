...
local generalOptions = {
    function(selectedPlayer) TriggerClientEvent('wasabi_ambulance:killPlayer', selectedPlayer.id) end,
    function(selectedPlayer) TriggerClientEvent('wasabi_ambulance:revive', selectedPlayer.id) end,
...
RegisterNetEvent('qbx_admin:server:playerOptionsGeneral', function(selected, selectedPlayer, input)
    if not IsPlayerAceAllowed(source, 'mod') then exports.qbx_core:Notify(source, locale('error.no_perms'), 'error') return end
...
local administrationOptions = {
    function(source, selectedPlayer, input)
        ...
        TriggerEvent('qb-log:server:CreateLog', 'bans', 'Player Kicked', 'red', string.format('%s was kicked by %s for %s', GetPlayerName(selectedPlayer.id), GetPlayerName(source), input), true)
    end,
    function(source, selectedPlayer, input)
        if not IsPlayerAceAllowed(source, 'admin') then exports.qbx_core:Notify(source, locale('error.no_perms'), 'error') return end
        ...
        TriggerEvent('qb-log:server:CreateLog', 'bans', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(selectedPlayer.id), GetPlayerName(source), input[1]), true)
    end,
    function(source, selectedPlayer, input)
        if not IsPlayerAceAllowed(source, 'admin') then exports.qbx_core:Notify(source, locale('error.no_perms'), 'error') return end
...
lib.callback.register('qbx_admin:server:clothingMenu', function(source, target)
    if not IsPlayerAceAllowed(source, 'mod') then
    ...
    TriggerClientEvent('qb-clothing:client:openMenu', target)
        ...
        TriggerEvent('qb-log:server:CreateLog', 'admin_cloth', '👗 Clothing Menu [Admin Menu]', 'blue', logMsg, false)
    return true
end)
...
local function hasWasabi()
    return GetResourceState('wasabi_ambulance') == 'started'
end

local function validTarget(id)
    return id and GetPlayerName(id) ~= nil
end

local function msg(src, text)
    if src > 0 then
        exports.qbx_core:Notify(src, text, 'inform')
        -- TriggerClientEvent('chat:addMessage', src, { args = { '^2SYSTEM', text } })
    else
        print(('[SYSTEM] %s'):format(text))
    end
end

-- /kill [id]
lib.addCommand('kill', {
    help = 'Kill yourself or a target player',
    params = { { name = 'id', type = 'number', help = 'target server id (optional)', optional = true } },
    restricted = 'group.mod'
}, function(source, args)
    if not hasWasabi() then return msg(source, 'wasabi_ambulance is not started.') end

    local target = tonumber(args.id) or source
    if not validTarget(target) then return msg(source, 'Invalid or offline target id.') end
    
    TriggerClientEvent('wasabi_ambulance:killPlayer', target)
    local Player = exports.qbx_core:GetPlayer(source)
    local TPlayer = exports.qbx_core:GetPlayer(tonumber(target))
    local logMsg = ('**ADMIN**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**TARGET**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s'):format(
                    GetPlayerName(source), 
                    Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname, 
                    Player.PlayerData.citizenid, 
                    Player.PlayerData.license, 
                    GetPlayerName(tonumber(target)), 
                    TPlayer.PlayerData.charinfo.firstname .. " " .. TPlayer.PlayerData.charinfo.lastname, 
                    TPlayer.PlayerData.citizenid, 
                    TPlayer.PlayerData.license 
                )
    TriggerEvent('qb-log:server:CreateLog', 'admin_kills', '☠️ Player Killed [Admin Menu]', 'red', logMsg, false)
    if source == target then
        msg(source, 'You have killed yourself.')
    else
        msg(source, ('Killed %s (%d).'):format(GetPlayerName(target), target))
    end
end)

-- /revive [id]
lib.addCommand('revive', {
    help = 'Revive yourself or a target player',
    params = { { name = 'id', type = 'number', help = 'target server id (optional)', optional = true } },
    restricted = 'group.mod'
}, function(source, args)
    if not hasWasabi() then return msg(source, 'wasabi_ambulance is not started.') end
    ...
    local Player = exports.qbx_core:GetPlayer(source)
        ...
        TriggerEvent('qb-log:server:CreateLog', 'admin_revive', '💝 Player Revive [Admin Menu]', 'orange', logMsg, false)
    if source == target then
        msg(source, 'You have revived yourself.')
    else
        msg(source, ('Revived %s (%d).'):format(GetPlayerName(target), target))
    end
end)
