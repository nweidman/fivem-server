...
RegisterNetEvent('qb-bossmenu:client:ManageEmployee', function(data)
    ...
    table.sort(EmployeeMenu, function(a, b)
        if a.isMenuHeader or b.isMenuHeader then
            return false -- Do not swap positions of headers
        end
        return a.params.args.grade > b.params.args.grade
    end)
    EmployeeMenu[#EmployeeMenu + 1] = {
...
RegisterNetEvent('qb-bossmenu:client:Stash', function()
    local src = source
    -- TriggerServerEvent('md-stashes:server:OpenStash', 'boss_' .. PlayerJob.name, 100000, 10)
