onResourceStart(function()
	...
	lib.addCommand('givecash', {
		...
		params = {
			{ name = 'playerid', type = 'number', help = 'Player ID' },
			{ name = 'amount', type = 'number', help = 'Amount of cash' }
		},
		...
		if not target or not amount then
			QBCore.Functions.Notify(src, 'Usage: /givecash [id] [amount]', "error")
			return
		end

		local other = exports.qbx_core:GetPlayer(target)
		if not other then return QBCore.Functions.Notify(src, 'Player not found!', "error") end

		if player.PlayerData.source == other.PlayerData.source then
			QBCore.Functions.Notify(src, "You can't give cash to yourself!", "error") 
			return
		end

		local dist = #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target)))
		if dist > 2.0 then
			QBCore.Functions.Notify(src, "The player is too far away!", "error") 
			return
		end

		if amount > 0 then
			if player.Functions.RemoveMoney('cash', amount, 'give-cash') then
				QBCore.Functions.Notify(src, "You have given the person $"..amount.." in cash!", "success") 
				QBCore.Functions.Notify(target, "You have received $"..amount.." in cash!", "success") 
				other.Functions.AddMoney('cash', amount, 'received-cash')
				TriggerClientEvent('qb-smallresources:client:giveCash', src)
                TriggerClientEvent('qb-smallresources:client:receiveCash', target)
			else
