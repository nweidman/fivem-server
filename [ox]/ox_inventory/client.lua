...
---@param ped number
---@return boolean
local function canOpenTarget(ped, targetId)
	...
	or (targetId and IsTargetDead(targetId))
end
...
function client.openInventory(inv, data)
    ...
    if inv == 'player' and data ~= cache.serverId then
        ...
        if not targetCoords or #(targetCoords - GetEntityCoords(playerPed)) > 2.8 or not canOpenTarget(targetPed, data) then
            return lib.notify({ id = 'inventory_right_access', type = 'error', description = locale('inventory_right_access') })
        end
    end
...
local function registerCommands()
	...
	lib.addKeybind({
		...
		defaultKey = 'F2',
		onPressed = function()
			print("DEBUG: Hotbar keybind pressed")
			...
			if not canOpenInventory() then
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					...
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > maxDistance or (not client.hasGroup(shared.police) and not canOpenTarget(ped)) then
							client.closeInventory()
...
RegisterNUICallback('uiLoaded', function(_, cb)
	...
	local themeSettings = {}
	local mainColor = GetResourceKvpString('ox_inventory_main_color')
	local darkText = GetResourceKvpString('ox_inventory_dark_text')
	local position = GetResourceKvpString('ox_inventory_position')
	local slots = GetResourceKvpString('ox_inventory_slots')
	local layoutMode = GetResourceKvpString('ox_inventory_layout_mode')

	if mainColor then
		themeSettings.mainColor = mainColor
	end
	if darkText then
		themeSettings.darkText = darkText
	end
	if position then
		themeSettings.position = position
	end
	if slots then
		themeSettings.slots = slots
	end
	if layoutMode then
		themeSettings.layoutMode = layoutMode
	end
	SendNUIMessage({
...
local givingItem = {}
local givingItemTimeout = {}

local function giveItemToTarget(serverId, slotId, count)
    if type(slotId) ~= 'number' then return end
    if count and type(count) ~= 'number' then return end

    -- Create a unique key for this operation
    local operationKey = ('%s:%s:%s'):format(serverId, slotId, count or 0)

    if givingItem[operationKey] then return end
    givingItem[operationKey] = true

    -- Clear the guard after 2 seconds
    if givingItemTimeout[operationKey] then
        ClearTimeout(givingItemTimeout[operationKey])
    end
    givingItemTimeout[operationKey] = SetTimeout(2000, function()
        givingItem[operationKey] = nil
        givingItemTimeout[operationKey] = nil
    end)
    ...
    local success = lib.callback.await('ox_inventory:giveItem', false, slotId, serverId, count or 0)

    if not success then
        lib.notify({ type = 'error', description = locale('cannot_give', count or 1, 'item') })
    end

    -- Clear immediately after callback completes
    givingItem[operationKey] = nil
    if givingItemTimeout[operationKey] then
        ClearTimeout(givingItemTimeout[operationKey])
        givingItemTimeout[operationKey] = nil
...
RegisterNUICallback('robPlayer', function(id, cb)
	TriggerServerEvent('police:server:RobPlayer', tonumber(id))
	cb(1)
end)

lib.callback.register('ox_inventory:startCrafting', function(recipe)
	local PlayerData = exports.qbx_core:GetPlayerData()
	local WorkLevel = PlayerData.metadata.skills.levels.working.tier
	local duration
	if WorkLevel then
		duration = 5000 - WorkLevel*1000
	elseif recipe.duration then
...
RegisterNUICallback('stealCash', function(targetId, cb)
	print('^3[DEBUG] stealCash called with targetId:', targetId)
	
	if not currentInventory or currentInventory.type ~= 'otherplayer' then
		print('^1[DEBUG] Failed: Not otherplayer inventory or no currentInventory')
		cb(false)
		return
	end
	
	print('^3[DEBUG] Current inventory type:', currentInventory.type, 'ID:', currentInventory.id)
	
	-- Use the passed target ID from the UI
	local target = tonumber(targetId)
	
	if not target then
		print('^1[DEBUG] Failed: Invalid target ID:', targetId)
		lib.notify({
			type = 'error',
	...
	print('^3[DEBUG] Calling server callback with target:', target)
	
	-- Call server callback to steal cash
	local success, amount = lib.callback.await('ox_inventory:stealCash', false, target)
	
	print('^3[DEBUG] Server callback result - success:', success, 'amount:', amount)
	
	if success then
		lib.notify({
			type = 'success',
			description = ('Successfully stole $%s'):format(amount)
		})
	else
		lib.notify({
			type = 'error',
			description = amount or 'Failed to steal cash'
		})
	end
...
RegisterNUICallback('reportPlayer', function(_, cb)
	cb(true)
	ExecuteCommand('report')
end)

RegisterNUICallback('emotemenu', function(_, cb)
	cb(true)
	ExecuteCommand('emotemenu')
end)

RegisterNUICallback('clothingAction', function(data, cb)
	cb(true)
...
RegisterCommand('resetinventoryformatting', function()
	DeleteResourceKvp('ox_inventory_layout_mode')
	SendNUIMessage({
		action = 'resetThemeSettings',
	})
end)

RegisterNUICallback('saveThemeSettings', function(data, cb)
	if data.mainColor then
		SetResourceKvp('ox_inventory_main_color', data.mainColor)
	end
	if data.darkText ~= nil then
		SetResourceKvp('ox_inventory_dark_text', tostring(data.darkText))
	end
	if data.position then
		SetResourceKvp('ox_inventory_position', data.position)
	end
	if data.slots then
		SetResourceKvp('ox_inventory_slots', data.slots)
	end
	if data.layoutMode then
		SetResourceKvp('ox_inventory_layout_mode', data.layoutMode)
	end
	cb(true)
end)
...
RegisterNUICallback('giveItemToTarget', function(data, cb)
    ...
    if not targetId or not slotId then return end

    -- Convert targetId to number (player inventories use server ID as the ID)
    targetId = tonumber(targetId)
    if not targetId then return end

    return giveItemToTarget(targetId, slotId, count)
end)
