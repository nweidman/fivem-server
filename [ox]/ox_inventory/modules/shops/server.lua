...
local function canAffordItem(inv, currency, price)
	local bankAccount = QBCore.Functions.GetPlayer(tonumber(inv.id)).PlayerData.money['bank']
	local canAfford = price >= 0 and bankAccount >= price
...
local function removeCurrency(inv, currency, price)
	QBCore.Functions.GetPlayer(tonumber(inv.id)).Functions.RemoveMoney('bank', price)
	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				if string.find(string.upper(fromItem.name), "WEAPON_") then
					exports.drx_mdt:CreateWeapon(metadata.serial, { source = source }, fromItem.name, fromItem.label)
				end
				if fromData.count then
