...
function Inventory.AddItem(inv, item, count, metadata, slot, cb, reason)
	if type(item) ~= 'table' then item = Items(item) end
	...
	if not Inventory.CanCarryItem(inv.id, item, count) then
	...
	if success then
		local player = QBCore.Functions.GetPlayer(tonumber(inv.id))
		...
		if player then
			...
			logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
				...
				TriggerEvent('qb-log:server:CreateLog', 'playerinventoryadd', 'Add Item', 'green', logMsg, false)

			local logsTable = {
				["steamname"] = player.PlayerData.name, -- This could be a Lua variable in your script
				["citizenid"] = player.PlayerData.citizenid, -- This could be a Lua variable in your script
				["ign"] = player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, -- This could be a Lua variable in your script
				["addremove"] = "add", -- This could be a Lua variable in your script
				["item"] = item.name, -- This could be a Lua variable in your script
				["quantity"] = count, -- You can also just enter raw strings
				["reason"] = addReason
			}
			exports["fortislogs"]:addLog("inventorylog", logsTable)
		end
	end
...
function Inventory.RemoveItem(inv, item, count, metadata, slot, ignoreTotal, strict, reason)
	if type(item) ~= 'table' then item = Items(item) end
	...
	if count > 0 then
		...
		if removed > 0 then
			...
			if player then
				...
				TriggerEvent('qb-log:server:CreateLog', 'playerinventoryremove', 'Remove Item', 'red', logMsg, false)
			
				local logsTable = {
					["steamname"] = player.PlayerData.name, -- This could be a Lua variable in your script
					["citizenid"] = player.PlayerData.citizenid, -- This could be a Lua variable in your script
					["ign"] = player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, -- This could be a Lua variable in your script
					["addremove"] = "remove", -- This could be a Lua variable in your script
					["item"] = item.name, -- This could be a Lua variable in your script
					["quantity"] = removed, -- You can also just enter raw strings
					["reason"] = removeReason
				}
				exports["fortislogs"]:addLog("inventorylog", logsTable)
			end
...
local function dropItem(source, playerInventory, fromData, data)
	...
	local PlayerData = QBCore.Functions.GetPlayer(tonumber(playerInventory.id)).PlayerData
	local logsTable = {
		["steamname"] = PlayerData.name, -- This could be a Lua variable in your script
		["citizenid"] = PlayerData.citizenid, -- This could be a Lua variable in your script
		["ign"] = PlayerData.charinfo.firstname .. " " .. PlayerData.charinfo.lastname, -- This could be a Lua variable in your script
		["addremove"] = "remove", -- This could be a Lua variable in your script
		["item"] = toData.name, -- This could be a Lua variable in your script
		["quantity"] = data.count, -- You can also just enter raw strings
		["reason"] = ('Dropped item to %s'):format(dropId)
	}
	exports["fortislogs"]:addLog("inventorylog", logsTable)

	local player = QBCore.Functions.GetPlayer(tonumber(playerInventory.id))
	local removeReason = ('Dropped item to "%s"'):format(dropId)
	logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
			player.PlayerData.name,
			player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, 
			player.PlayerData.citizenid, 
			...
			if toData and ((toData.name ~= fromData.name) or not toData.stack or (not table.matches(toData.metadata, fromData.metadata))) then
				...
				if not sameInventory then
					if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
						...
						local PlayerData = QBCore.Functions.GetPlayer(tonumber(playerInventory.id)).PlayerData
						local reason = 'Transferred from %s to %s'
						if fromInventory.owner and toInventory.owner then 
							reason = 'Robbed from %s to %s'
							
							if playerInventory.owner ~= fromInventory.owner then
								local PlayerData2 = QBCore.Functions.GetPlayer(tonumber(fromInventory.id)).PlayerData
								local logsTable = {
									["steamname"] = PlayerData2.name, -- This could be a Lua variable in your script
									["citizenid"] = PlayerData2.citizenid, -- This could be a Lua variable in your script
									["ign"] = PlayerData2.charinfo.firstname .. " " .. PlayerData2.charinfo.lastname, -- This could be a Lua variable in your script
									["addremove"] = "remove", -- This could be a Lua variable in your script
									["item"] = toData.name, -- This could be a Lua variable in your script
									["quantity"] = toData.count, -- You can also just enter raw strings
									["reason"] = reason:format(fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id)
								}
								exports["fortislogs"]:addLog("inventorylog", logsTable)
								...
								local logsTable2 = {
									...
									["ign"] = PlayerData2.charinfo.firstname .. " " .. PlayerData2.charinfo.lastname, -- This could be a Lua variable in your script
									["addremove"] = "add", -- This could be a Lua variable in your script
									["item"] = fromData.name, -- This could be a Lua variable in your script
									["quantity"] = fromData.count, -- You can also just enter raw strings
									["reason"] = reason:format(toInventory.owner and toInventory.label or toInventory.id, fromInventory.owner and fromInventory.label or fromInventory.id)
								}
								exports["fortislogs"]:addLog("inventorylog", logsTable2)
							else
								local PlayerData2 = QBCore.Functions.GetPlayer(tonumber(toInventory.id)).PlayerData
								local logsTable = {
									["steamname"] = PlayerData2.name, -- This could be a Lua variable in your script
									["citizenid"] = PlayerData2.citizenid, -- This could be a Lua variable in your script
									["ign"] = PlayerData2.charinfo.firstname .. " " .. PlayerData2.charinfo.lastname, -- This could be a Lua variable in your script
						...
						if fromInventory.owner == PlayerData.citizenid then
							local logsTable = {
								["steamname"] = PlayerData.name, -- This could be a Lua variable in your script
								["citizenid"] = PlayerData.citizenid, -- This could be a Lua variable in your script
								["ign"] = PlayerData.charinfo.firstname .. " " .. PlayerData.charinfo.lastname, -- This could be a Lua variable in your script
							...
							local player = QBCore.Functions.GetPlayer(tonumber(playerInventory.id))
							local addReason = reason:format(toInventory.owner and toInventory.label or toInventory.id, fromInventory.owner and fromInventory.label or fromInventory.id)
							logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
									player.PlayerData.name,
									player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, 
									player.PlayerData.citizenid, 
							...
							local removeReason = reason:format(fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id)
							logMsg2 = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
									player.PlayerData.name,
									player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, 
									player.PlayerData.citizenid, 
							...
							TriggerEvent('qb-log:server:CreateLog', 'playerinventoryremove', 'Remove Item', 'red', logMsg2, false)
							...
							local player = QBCore.Functions.GetPlayer(tonumber(playerInventory.id))
							local addReason = reason:format(fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id )
							logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
									player.PlayerData.name,
									player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, 
									player.PlayerData.citizenid, 
							...
							local removeReason = reason:format(toInventory.owner and toInventory.label or toInventory.id, fromInventory.owner and fromInventory.label or fromInventory.id)
							logMsg2 = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
									player.PlayerData.name,
									player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, 
									player.PlayerData.citizenid, 
				...
				if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
					...
					if not sameInventory then
						...
						local logsTable = {
							...
							["citizenid"] = PlayerData.citizenid, -- This could be a Lua variable in your script
							["ign"] = PlayerData.charinfo.firstname .. " " .. PlayerData.charinfo.lastname, -- This could be a Lua variable in your script
							["addremove"] = toInventory.id ~= playerInventory.id and "remove" or "add", -- This could be a Lua variable in your script
							["item"] = fromData.name, -- This could be a Lua variable in your script
							["quantity"] = data.count, -- You can also just enter raw strings
							["reason"] = ('Transferred from %s to %s'):format(fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id)
						}
						exports["fortislogs"]:addLog("inventorylog", logsTable)

						if toInventory.id ~= playerInventory.id then
							local player = QBCore.Functions.GetPlayer(tonumber(playerInventory.id))
							local removeReason = ('Transferred from %s to %s'):format(toInventory.owner and toInventory.label or toInventory.id, fromInventory.owner and fromInventory.label or fromInventory.id)
							local logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
									player.PlayerData.name,
									player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, 
									player.PlayerData.citizenid, 
							...
							local player = QBCore.Functions.GetPlayer(tonumber(playerInventory.id))
							local addReason = ('Transferred from %s to %s'):format(toInventory.owner and toInventory.label or toInventory.id, fromInventory.owner and fromInventory.label or fromInventory.id)
							local logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
									player.PlayerData.name,
									player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, 
									player.PlayerData.citizenid, 
			...
			elseif data.count <= fromData.count then
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						local reason = 'Transferred from %s to %s'
						local P1 = QBCore.Functions.GetPlayer(fromInventory.id)
						local P2 = QBCore.Functions.GetPlayer(toInventory.id)
						if P1 and P2 then 
							reason = 'Robbed from %s to %s' 
							local PlayerData = QBCore.Functions.GetPlayer(tonumber(toInventory.id)).PlayerData
							local PlayerData2 = QBCore.Functions.GetPlayer(tonumber(fromInventory.id)).PlayerData
								local logsTable = {
									["steamname"] = PlayerData2.name, -- This could be a Lua variable in your script
									["citizenid"] = PlayerData2.citizenid, -- This could be a Lua variable in your script
						...
						if toInventory.id ~= playerInventory.id then
							local player = QBCore.Functions.GetPlayer(tonumber(playerInventory.id))
							local removeReason = ('Transferred from %s to %s')
							if P1 and P2 then removeReason = 'Robbed from %s to %s' end
							local logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
									player.PlayerData.name,
									player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, 
									player.PlayerData.citizenid, 
									...
									fromData.name,
									data.count,
									removeReason:format(toInventory.owner and toInventory.label or toInventory.id, fromInventory.owner and fromInventory.label or fromInventory.id)
							)
							TriggerEvent('qb-log:server:CreateLog', 'playerinventoryremove', 'Remove Item', 'red', logMsg, false)
						else
							local player = QBCore.Functions.GetPlayer(tonumber(playerInventory.id))
							local addReason = ('Transferred from %s to %s')
							if P1 and P2 then addReason = 'Robbed from %s to %s' end
							local logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID: **%s\n**License: **%s\n\n**ITEM**\n**Name:** %s\n**Amount:** %s\n**Reason:** %s'):format(
									player.PlayerData.name,
									player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, 
									player.PlayerData.citizenid, 
									...
									fromData.name,
									data.count,
									addReason:format(toInventory.owner and toInventory.label or toInventory.id, fromInventory.owner and fromInventory.label or fromInventory.id)
							)
							TriggerEvent('qb-log:server:CreateLog', 'playerinventoryadd', 'Add Item', 'green', logMsg, false)
						end
					end
...
function Inventory.Clear(inv, keep)
	...
	local player = QBCore.Functions.GetPlayer(tonumber(inv.id))
	local logMsg = ('**PLAYER**\n**Name:** %s\n**Character Name:** %s\n**Citizen ID:** %s\n**License:** %s'):format(GetPlayerName(inv.id), player.PlayerData.charinfo.firstname .. " " .. player.PlayerData.charinfo.lastname, player.PlayerData.citizenid, QBCore.Functions.GetIdentifier(inv.id, 'license')..'\n\n**CLEARED ITEMS**\n'..json.encode(inv.items))
	TriggerEvent('qb-log:server:CreateLog', 'playerinventoryclear', 'Inventory Cleared', 'red', logMsg, false)
	local updateSlots = {}
	local newWeight = 0
	local inc = 0
...
local activeGiveOperations = {}

local function giveItem(playerId, slot, target, count)
	-- Use a simpler key that doesn't depend on count to prevent duplicates
	local operationKey = ('%s:%s:%s'):format(playerId, slot, target)

	if activeGiveOperations[operationKey] then
		return false
	end

	activeGiveOperations[operationKey] = true

	-- Defer clearing the guard until after the operation completes
	local function clearGuard()
		CreateThread(function()
			Wait(500)
			activeGiveOperations[operationKey] = nil
		end)
	end
	...
	if not toInventory or not toInventory.player then
		clearGuard()
		return false
	end

	local itemData = fromInventory.items[slot]
	if not itemData then
		clearGuard()
		return false
	end

	if count <= 0 then count = itemData.count end
	-- Cap count to the actual item count if it's greater
	if count > itemData.count then
		count = itemData.count
	end

	local item = Items(itemData.name)
	if not item then
		clearGuard()
		return false
	end
	...
	if not Inventory.CanCarryItem(toInventory, item, count, itemData.metadata) then
		clearGuard()
		return false
	end

	-- Check distance between players
	local fromPed = fromInventory.player.ped
	local toPed = toInventory.player.ped
	if fromPed and toPed then
	...
	-- Check if target inventory is busy (unless opened by source)
	local isOpenBySource = toInventory.openedBy and toInventory.openedBy[fromInventory.id]
	if not isOpenBySource then
		local success, targetState = pcall(function() return Player(target).state end)
		if success and targetState and targetState.invBusy then
			clearGuard()
			return false
		end
	end

	-- Find slot in target inventory
	local toSlot = Inventory.GetSlotForItem(toInventory, itemData.name, itemData.metadata)

	-- If GetSlotForItem didn't find a slot, try to get an empty slot
	if not toSlot then
		toSlot = Inventory.GetEmptySlot(toInventory)
	end
	...
	-- Verify item still exists and has enough count before transferring
	local currentItemData = fromInventory.items[slot]
	if not currentItemData or currentItemData.name ~= itemData.name or currentItemData.count < count then
		clearGuard()
		return false
	end

	-- Transfer the item
	local addSuccess = Inventory.AddItem(toInventory, item, count, itemData.metadata, toSlot)
	if addSuccess then
		-- Double-check item still exists before removing
		local verifyItem = fromInventory.items[slot]
		if verifyItem and verifyItem.name == itemData.name and verifyItem.count >= count then
			local removeSuccess = Inventory.RemoveItem(fromInventory, item, count, itemData.metadata, slot)
			if removeSuccess then
				if server.loglevel > 0 then
					lib.logger(fromInventory.owner, 'giveItem', ('"%s" gave %sx %s to "%s"'):format(fromInventory.label, count, itemData.name, toInventory.label))
				end
				clearGuard()
				return true
			else
				-- Rollback if remove failed
				Inventory.RemoveItem(toInventory, item, count, itemData.metadata, toSlot)
			end
		else
			-- Item was already removed, rollback
			Inventory.RemoveItem(toInventory, item, count, itemData.metadata, toSlot)
		end
	end
...
lib.callback.register('ox_inventory:giveItem', function(source, slot, target, count)
	return giveItem(source, slot, target, count)
end)
...
local function registerStash(name, label, slots, maxWeight, owner, groups, coords)
	if string.find(name, "hobo_recycler") then
		maxWeight = 1000000
	end
