...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	if data then
		...
		elseif invType == 'player' then
			right = Inventory(data)
			if right then
				right.player = { ped = GetPlayerPed(data) }
			end
...
lib.addCommand({'additem', 'giveitem'}, {
	...
	if item then
		...
		local TPlayer = exports.qbx_core:GetPlayer(tonumber(args.target))
		local Player = exports.qbx_core:GetPlayer(src)
			...
			TriggerEvent('qb-log:server:CreateLog', 'admin_giveitem', 'ℹ️ /giveitem', 'green', logMsg, false)
...
lib.addCommand('markedbills', {
	help = 'Gives yourself markedbills with the given amount',
	params = {
		{ name = 'amount', type = 'number', help = 'The amount of money to put on the bill', optional = false },
	},
	restricted = 'group.mod',
}, function(source, args)
	...
	exports.ox_inventory:AddItem(src, 'markedbills', 1, {
		...
		local Player = exports.qbx_core:GetPlayer(src)
			...
			TriggerEvent('qb-log:server:CreateLog', 'admin_givemoney', '💰 /markedbills', 'green', logMsg, false)
end)
...
lib.addCommand('removeitem', {
	...
	if item then
		...
		TriggerEvent('qb-log:server:CreateLog', 'admin_giveitem', 'ℹ️ /removeitem', 'red', logMsg, false)
		if server.loglevel > 0 then
...
lib.callback.register('ox_inventory:stealCash', function(source, targetId)
	print('^3[DEBUG] Server stealCash called - source:', source, 'targetId:', targetId)
	
	local Player = exports.qbx_core:GetPlayer(source)
	local TargetPlayer = exports.qbx_core:GetPlayer(targetId)
	
	print('^3[DEBUG] Player found:', Player and 'yes' or 'no', 'TargetPlayer found:', TargetPlayer and 'yes' or 'no')
	
	if not Player or not TargetPlayer then
		print('^1[DEBUG] Failed: Player not found')
		return false, 'Player not found'
	end
	
	-- Get cash amount first
	local stealAmount = TargetPlayer.PlayerData.money.cash or 0
	print('^3[DEBUG] Target cash amount:', stealAmount)
	
	if stealAmount <= 0 then
		print('^1[DEBUG] Failed: Target has no cash to steal')
		return false, 'Target has no cash to steal'
	end
	
	-- Transfer money
	TargetPlayer.Functions.RemoveMoney('cash', stealAmount)
	Player.Functions.AddMoney('cash', stealAmount)
	
	print('^2[DEBUG] Successfully stole $' .. stealAmount)
	return true, stealAmount
end)

lib.callback.register('uiLoaded', function(source)
    return true
end)