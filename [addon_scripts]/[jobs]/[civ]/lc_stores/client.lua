...
local function getRearCargoPoint(vehicle)
	if not vehicle or not DoesEntityExist(vehicle) then return nil end

	local model = GetEntityModel(vehicle)
	local minDim, maxDim = GetModelDimensions(model)

	local candA = GetOffsetFromEntityInWorldCoords(vehicle, 0.0, (minDim.y or -2.0), 0.0)
	local candB = GetOffsetFromEntityInWorldCoords(vehicle, 0.0, (maxDim.y or  2.0), 0.0)

	local vehPos = GetEntityCoords(vehicle)
	local fwd = GetEntityForwardVector(vehicle)

	local function dotBehind(worldPoint)
		local dir = worldPoint - vehPos
		return (dir.x * fwd.x) + (dir.y * fwd.y) + (dir.z * fwd.z)
	end

	local scoreA = dotBehind(candA)
	local scoreB = dotBehind(candB)

	local rearLocalY = (scoreA < scoreB) and (minDim.y or -2.0) or (maxDim.y or 2.0)

	rearLocalY = rearLocalY - 0.75

	local midZ = ((minDim.z + maxDim.z) * 0.5)
	if not midZ or midZ == 0.0 then midZ = 0.25 end

	return GetOffsetFromEntityInWorldCoords(vehicle, 0.0,  rearLocalY, midZ)
end
...
AddEventHandler('stores:startJob', function(truck_level,is_import)
	...
	while IsEntityAVehicle(truck_vehicle) do
		...
		elseif delivery_phase == 2 then
			if is_import then
				local vehicle_trunk = getRearCargoPoint(truck_vehicle)
				if vehicle_trunk then
					local distance = #(GetEntityCoords(ped) - vehicle_trunk)

					if distance <= 50 then
						timer = 2
						Utils.Markers.drawMarker(39, vehicle_trunk.x, vehicle_trunk.y, vehicle_trunk.z + 0.15, 1.0)

						if distance <= 2.8 then
							Utils.Markers.drawText3D(vehicle_trunk.x, vehicle_trunk.y, vehicle_trunk.z + 0.55, Utils.translate('objective_marker_2'))
							if IsControlJustPressed(0,38) then
								if not (IsPedSittingInAnyVehicle(ped) or IsPedInAnyVehicle(ped, true)) then
									deleteObject(object)
									route_blip = Utils.Blips.createBlipForCoords(garage_coord.x,garage_coord.y,garage_coord.z,Config.route_blip.id,Config.route_blip.color,Utils.translate('blip_route'),0.8,true)
		...
		if has_error then
			deleteObject(object)
			Utils.Vehicles.removeKeysFromPlate(properties.plate,truck_model)
			Utils.Blips.removeBlip(truck_blip)
			Utils.Blips.removeBlip(route_blip)
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", false)
