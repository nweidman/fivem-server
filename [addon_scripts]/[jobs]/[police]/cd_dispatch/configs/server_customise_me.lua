...
local function UpdateDispatchRadioChannel(src, radio_channel)
    if not self or not self[src] then return end
    if radio_channel == nil then return end
    if not CheckJob(src, self[src].job) then return end

    self[src].radio_channel = tonumber(radio_channel) or 0
    RefreshLargeUI(self[src].job)
    PlayerBlipsActions(src, 'update')
end

RegisterServerEvent('cd_dispatch:GetRadioChannel')
AddEventHandler('cd_dispatch:GetRadioChannel', function(radio_channel)
    UpdateDispatchRadioChannel(source, radio_channel)
end)

RegisterServerEvent('dispatch:GetRadioChannel')
AddEventHandler('dispatch:GetRadioChannel', function(radio_channel)
    UpdateDispatchRadioChannel(source, radio_channel)
end)
...
function Notification(source, notif_type, message)
    if source and notif_type and message then
        ...
        elseif Config.Notification == 'cd_notifications' then
            if notif_type == 1 then
                TriggerClientEvent('cd_notifications:Add', source, {title =  L('dispatch'), message = message, type = 'success', options = {duration = 5}})
            elseif notif_type == 2 then
                TriggerClientEvent('cd_notifications:Add', source, {title =  L('dispatch'), message = message, type = 'inform', options = {duration = 5}})
            elseif notif_type == 3 then
                TriggerClientEvent('cd_notifications:Add', source, {title =  L('dispatch'), message = message, type = 'error', options = {duration = 5}})
            end

        elseif Config.Notification == 'okokNotify' then
            if notif_type == 1 then
                TriggerClientEvent('okokNotify:Alert', source, L('dispatch'), message, 5000, 'success')
            elseif notif_type == 2 then
                TriggerClientEvent('okokNotify:Alert', source, L('dispatch'), message, 5000, 'info')
            elseif notif_type == 3 then
                TriggerClientEvent('okokNotify:Alert', source, L('dispatch'), message, 5000, 'error')
            end
        ...
        elseif Config.Notification == 'ox_lib' then
            if notif_type == 1 then
                lib.notify(source, {title = L('dispatch'), description = message, type = 'success'})
            elseif notif_type == 2 then
                lib.notify(source, {title = L('dispatch'), description = message, type = 'inform'})
            elseif notif_type == 3 then
                lib.notify(source, {title = L('dispatch'), description = message, type = 'error'})
            end
...
function PlayerDropped(source)
    if self and source and self[source] then
        ...
        if self[source].dispatcher then
            TriggerEvent('cd_dispatch:DispatcherToggle', false, self[source].job, self[source])
        end
...
AddEventHandler('cd_dispatch:AddNotification', function(data)
    local src = source
    if AntiCheatTriggered(src, data.job_table, {data.title, data.message, (data.blip and data.blip.text)}) then return end
...
RegisterCommand('refreshblips', function(source, args, rawCommand)
    local src = source
    -- Check if the player running the command is an admin (optional)
    local Player = QBCore.Functions.GetPlayer(src) -- or adapt for ESX framework
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "sheriff" or Player.PlayerData.job.name == "ambulance"  then
        -- Notify all players to refresh their blips
        TriggerClientEvent('cd_dispatch:refreshBlips', -1)
        -- print("^2Blips have been refreshed for all players.^7")
        -- TriggerClientEvent('chatMessage', src, "^2Blips have been refreshed.^7")
    else
        TriggerClientEvent('chatMessage', src, "^1You do not have permission to use this command.^7")
    end
end, false)
