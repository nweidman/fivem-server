...
RegisterNetEvent('HD_Jail:sendToJail', function(target, time, reason)
    local playerId = source

    if isResourcePresentProvideless('streetcode-AICopsSystem') then
        exports['rcore_prison']:Jail(playerId, time, reason)
    end
end)
...
RegisterNetEvent('al_mdt:JailPlayer', function(_, target, reason, jailTime)
    JailPlayer(source, target, jailTime, 'identifier')
end)
