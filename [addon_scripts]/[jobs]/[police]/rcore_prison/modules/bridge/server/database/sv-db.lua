...
db.UpdateJailData = function(data, charId, callback)
    MySQL.Async.execute(db.Queries.GAME_STRUCTURE.UPDATE_JAIL_DATA, { json.encode(data), charId }, function(result)
        if callback then
            callback(result)
        end
    end)
...
db.GetServerItems = function()
    return MySQL.Sync.fetchAll(db.Queries.FRAMEWORK.FETCH_ITEMS, {})
end
