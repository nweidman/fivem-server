CreateThread(function()
    if Config.Inventories == Inventories.CORE then
        ...
        Inventory.getInventoryItems = function(playerId)
            ...
            if not doesExportExistInResource(Inventories.CORE, 'getInventory') then
                dbg.critical('Inventory: Failed to find export named getInventory in your %s - update your inventory!', Config.Inventories)
                return {}
            end

            if Config.Framework == Framework.ESX and player then
                content = exports[Inventories.CORE]:getInventory('content-' .. player.getIdentifier():gsub(':',''))
                primary = exports[Inventories.CORE]:getInventory('primary-' .. player.getIdentifier():gsub(':',''))

                inventory = table.merge(content, primary)
            end

            if Config.Framework == Framework.QBCore and player then
                content = exports[Inventories.CORE]:getInventory('content-' .. player.PlayerData.citizenid)
                primary = exports[Inventories.CORE]:getInventory('primary-' .. player.PlayerData.citizenid)
