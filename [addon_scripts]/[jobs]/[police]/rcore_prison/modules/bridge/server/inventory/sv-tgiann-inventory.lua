...
CreateThread(function()
    if Config.Inventories == Inventories.TGIANN then
        ...
        Inventory.DoesItemExist = function(itemName, playerId)
            if doesExportExistInResource(Inventories.TGIANN, 'Items') then
                local itemData = exports[Inventories.TGIANN]:Items(itemName)
        ...
        Inventory.clearInventory = function(client)
            ...
            if doesExportExistInResource(Config.Inventories, "ClearInventory") then
                exports[Config.Inventories]:ClearInventory(client, "money_item")
                retval = true
            end
