...
RegisterNetEvent('evidence:server:SetGSR', function(bool, citizenid)
    ...
    if bool then
        PlayerGSR[cid] = os.time() + (60 * Config.GSR.FadeTime)
    else
        PlayerGSR[cid] = nil
    end
end)
...
Config.Functions.RegisterCallback('police:GetVinScratched', function(source, cb, plate) -- Check for vinscratched value in DB
    local scratched = false

    if plate then
        MySQL.Async.fetchScalar('SELECT vinscratched FROM player_vehicles WHERE plate = @plate', {
            ['@plate'] = plate
        }, function(result)
            if result == 1 then
                scratched = true
            ...
            if Config.Debug.PrintTest.enabled then
                print(('Vehicle VIN inspection for %s returns %s'):format(plate, scratched))
            end
...
CreateThread(function()
    while true do
        ...
        for k, v in pairs(srclist) do
            ...
            Wait(500) -- slow down for loop for less resource usage
        end 
