...
RegisterNetEvent('evidence:server:updateimagespace', function(photo, remove)
    local src = source
    ...
    if not remove then
        ...
        if Webhooks[photo.uploadPath] and Webhooks[photo.uploadPath] ~= '' then
            local payload = {
                content = "",
                tts = false,
                embeds = {
                ...
                actions = {},
                flags = 0,
                username = "[BOT] PD Camera",
                avatar_url = "https://cdn.discordapp.com/avatars/1246250101295284296/fd399170ef7af13535b3707844335062.png?size=512"
            }

            PerformHttpRequest(Webhooks[photo.uploadPath], function(err, text, headers)
                if err ~= 200 and err ~= 204 then
                    print(('[r14-evidence] Discord webhook failed for uploadPath %s. Status: %s Response: %s'):format(
                        tostring(photo.uploadPath), tostring(err), tostring(text)
                    ))
                end
            ...
            print(('[r14-evidence] Missing Discord webhook for uploadPath %s'):format(tostring(photo.uploadPath)))
        end

        Config.Functions.ServerNotify(src, (Config.Notification.UploadedToImageSpace):format(imageSpace), "success")
    else
        if removeInProgress then
            TriggerClientEvent('nikon:client:canceldelete', src, 'System is already processing delete, please try again!')
            return
        else
            if not ImageSpaces[remove.imageSpace] or not ImageSpaces[remove.imageSpace][remove.photoId] then
                TriggerClientEvent('nikon:client:canceldelete', src, 'System could not locate photo, please try again!')
                return
            else
...
RegisterNetEvent('evidence:server:uploadphoto', function(uploadPath, photo)
    ...
    if uploadPath == 1 or not photo or photo.uploadPath == 1 then
        print(("%s attempted to upload image to local storage via %s??? %s"):format(
            src,
            GetInvokingResource() or 'client',
            json.encode(photo or {})
        ))
        return
    end
    ...
    if Webhooks[uploadPath] and Webhooks[uploadPath] ~= '' then
        local payload = {
            content = "",
            tts = false,
            embeds = {
        ...
        PerformHttpRequest(Webhooks[uploadPath], function(err, text, headers)
            if err ~= 200 and err ~= 204 then
                print(('[r14-evidence] Discord webhook failed for uploadPath %s. Status: %s Response: %s'):format(
                    tostring(uploadPath), tostring(err), tostring(text)
                ))
            end
        end, 'POST', json.encode(payload), {
            ['Content-Type'] = 'application/json'
        ...
        print(('[r14-evidence] Missing Discord webhook for uploadPath %s'):format(tostring(uploadPath)))
    end
end)
...
Config.Functions.CreateUseableItem("nekon", function(source, item, inv, slot, data)
    local src = source
    local items = nil
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name ~= 'police' then return end
    if Config.Inventory.Ox and false then item = inv.items[slot] src = inv.id if type(source) == 'string' and source ~= 'usingItem' then return end end  
...
Config.Functions.RegisterCallback('nikon:GetWebhook', function(source, cb, uploadPath)
    ...
    if uploadPath then
        if not Webhooks[uploadPath] then
            print(('%s attempted to access a camera webhook for image space %s that does not exist, please ensure your main config and server webhooks table match.'):format(source, uploadPath))
            webhook = 'missing'
            cb(webhook)
            return
        end

        if Webhooks[uploadPath] == '' then
            print(('%s attempted to access a blank camera webhook for image space %s, please check your server webhooks table.'):format(source, uploadPath))
            webhook = 'blank'
            cb(webhook)
            return
        end
