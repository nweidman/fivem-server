    ...
    if not casingtimeout and shooter == ped then
        ...
        if not Config.NoAlertWeapon[weapon] then
            ...
            elseif weapon == `weapon_firework` then
                ...
                elseif not IsPedInAnyVehicle(ped) and not exports["pug-paintball"]:IsInPaintball() then
...
RegisterNetEvent('evidence:client:CreateFingerprint', function(coords)
    ...
    coords = coords or GetOffsetFromEntityInWorldCoords(ped, 0.0, 0.25, 0.0)

    local hands = GetPedDrawableVariation(ped, 3)
    local model = GetEntityModel(ped) & 0xFFFFFFFF

    if not (Config.NoGloves[model] and Config.NoGloves[model][hands]) then
        return
    end

    TriggerServerEvent('evidence:server:CreateFingerprint', coords)
end)
...
RegisterNetEvent('evidence:client:checkvin', function(data)
    if Config.Debug.PrintTestC.enabled then
        print(("Check VIN event triggered for %s: %s"):format(Config.Functions.GetPlate(data.entity), json.encode(data)))
    end

    Config.Functions.Progressbar("checking", "Checking VIN of vehicle...", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        if #(GetEntityCoords(ped) - GetEntityCoords(data.entity)) < 10 then
            TriggerEvent('evidence:client:checkvinresult', data)
        else
            Config.Functions.Notify(Config.Notification.Evading, 'error')                    
        end
    end, function() -- Cancel
...
RegisterNetEvent('evidence:client:checkvinresult', function(data)
    local target = Config.Functions.GetPlate(data.entity)
    local scratched = false
    local complete = false

    if ActiveLEO then
        Config.Functions.TriggerCallback('police:GetVinScratched', function(result)
            if result then
                Config.Functions.Chat({
                    color = {255, 0, 0},
                    multiline = false,
                    args = {"VIN Check:", ("The VIN appears to have been scratched off."):format(target)}
                })
                scratched = true
            end
            complete = true
...
RegisterNetEvent('evidence:client:GSRtest', function(data)
    ...
    Config.Functions.Progressbar("gsrtest", "Performing GSR test...", 4000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:DNAswab', function(data)
    ...
    Config.Functions.Progressbar("dnaswab", "Performing DNA Swab...", 5000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
CreateThread(function()
    ...
    AddGlobalVehicleTargets({
        options = {
            ...
            { 
                type = "client",
                event = 'evidence:client:checkvin',
                icon = "fas fa-car-side",
                label = "Check VIN Number",
			    entity = entity,
                canInteract = function(entity, distance, data)
                    local auth = table.clone(Config.AuthorizedJobs)
                    local checkdead = Config.Functions.CheckDead
