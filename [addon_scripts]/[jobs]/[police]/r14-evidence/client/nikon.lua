...
function TakePhoto()
    ...
    exports['screenshot-basic']:requestScreenshotUpload(
        ...
        {
            headers = {
                Authorization = "SrWiL2NsL5fXoEphCMZczU5xhdEWAZqt"
            }
        },
        function(data)
            ...
            if not resp or not resp.data or not resp.data.url then
                print('[r14-evidence] FiveManage upload failed or returned invalid response')
                print(data or 'nil')
                Config.Functions.Notify('Unable to save photo, upload failed!', 'error')
            ...
            photodata = {
                url = resp.data.url,
                id = resp.data.id or tostring(math.random(100000, 999999)),
                time = ostime,
                ...
                exif = {
                    ...
                    [4] = {label = 'Uploaded:', value = deviceSettings.uploadPath ~= 1 and displaydate or false, order = 4},
