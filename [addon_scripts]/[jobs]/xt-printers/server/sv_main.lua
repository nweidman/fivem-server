...
lib.callback.register('xt-printers:server:refillPrinter', function(source, id, storageType)
    ...
    if exports.ox_inventory:RemoveItem(source, item, removeAmount) then
        ...
        lib.notify(source, {
            title = 'Missing Items!',
            description = 'You are missing items.',
            type = 'error'
        })
    end
...
lib.callback.register('xt-printers:server:getPrintersByJob', function(source)
    ...
    return DB.getPrintersByJob(job, Player.PlayerData.citizenid)
end)

-- new printer
lib.callback.register('xt-printers:server:createPrinter', function(source, info)
    -- local hasAllowedJob = UTILS.hasAllowedJob(source, info.owner)
    -- if not hasAllowedJob then return end
    local Player = exports.qbx_core:GetPlayer(source)
    if Player.Functions.RemoveItem('printer', 1) then
...
lib.callback.register('xt-printers:server:deletePrinter', function(source, id)
    ...
    -- local hasAllowedJob = UTILS.hasAllowedJob(source, printer.owner)
    -- if not hasAllowedJob then return end
    local Player = exports.qbx_core:GetPlayer(source)
    Player.Functions.AddItem('printer', 1)
    if printer.storage.paper >0 then
        Player.Functions.AddItem('printerpaper', printer.storage.paper)
    end
