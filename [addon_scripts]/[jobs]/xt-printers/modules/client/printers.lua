...
-- create printer
function PRINTER_UTILS.createPrinter(id, model, coords, heading, owner, public)
    ...
    local interactions = {
        -- public and group interactions
        {
            label = 'Print',
            icon = 'fas fa-print',
            groups = public == 0 and owner or false,
            onSelect = function()
                ...
                if not canPrint then
                    lib.notify({
                        title = 'No Paper or Ink!',
                        description = 'Printer Lacks Enough Paper or Ink!',
                        type = 'error'
                    })
                    return
                end
                ...
                if lib.progressCircle({
                    ...
                    lib.notify({
                        ...
                        description = 'The printing is complete',
                        type = 'success'
                    })
                end
            end
        },
        {
            ...
            onSelect = function()
                ...
                lib.notify({
                    ...
                    description = 'You have received the completed prints',
                    type = 'success'
                })
            end
        },
        ...
        },
        {
            ...
            onSelect = function()
                local deleted = lib.callback.await('xt-printers:server:deletePrinter', false, id)
                if not deleted then return end

                lib.notify({
                    title = 'Picked up Printer',
                    description = 'You have picked up the printer',
                    type = 'success'
                })
            end
        },
...
function PRINTER_UTILS.createAllPrinters()
    ...
    for id, info in pairs(getPrinters) do
        PRINTER_UTILS.createPrinter(id, info.model, info.coords.xyz, info.coords.w, info.owner, info.public)
    end
end
