...
local strings = {
    ...
    NEW_PRINTER = 'INSERT INTO `xt_printers` (`model`, `coords`, `owner`, `storage`, `public`) VALUES (?, ?, ?, ?, ?)',
...
local function cachePrinters()
    ...
    for x = 1, #getPrinters do
        local printergroup = getPrinters[x].owner
...
-- get printer by job
function db.getPrintersByJob(job, citizenid)
    local printersByJob = {}

    for id, info in pairs(cachedPrinters) do
        if info.owner == citizenid then
