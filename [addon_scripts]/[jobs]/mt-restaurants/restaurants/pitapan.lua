...
local restaurantConfig = { -- Auto-assigns restaurant ID based on job field
    ...
    label = 'Pita Pan', -- The display name of the restaurant
    job = 'pitapan', -- The job of the restaurant (will become the restaurant ID)
    ...
    blip = { enabled = true, coords = vec3(253.14, -819.69, 30.2), sprite = 439, color = 46, scale = 0.7, display = 4 }, -- The restaurant map blip
    zone = { -- The restaurant exterior zone
        points = {
            vec3(235.19, -826.17, 30.14),
            vec3(255.68, -833.8, 29.52),
            vec3(273.67, -807.33, 30.42),
            vec3(245.62, -796.72, 30.58),
        },
        thickness = 15,
    },
    management = { -- The restaurant boss menus (needed to create recipes)
        { coords = vec3(254.54, -813.9, 30.5), radius = 0.5 },
        { coords = vec3(256.14, -818.0, 30.2), radius = 0.5 },
    },
    stashes = { -- The restaurant stashes (blocked to the job)
        { coords = vec3(249.34, -811.65, 30.2), radius = 0.5, label = 'Ingredients Fridge', slots = 500, weight = 1000000 },
        { coords = vec3(254.25, -817.26, 30.2), radius = 1.0, label = 'Food Storage', slots = 500, weight = 1000000 },
    },
    trays = { -- The retaurant tray stashes (not blocked to any job)
        { coords = vec3(257.34, -817.41, 30.2), radius = 0.5, label = 'Food Tray', slots = 10, weight = 100000 },
    },
    ...
    stations = { -- The restaurant stations (Where you'll be able to prepare the made recipes)
        { coords = vec3(252.96, -813.73, 30.2), radius = 1.0, label = 'Pizza Oven', type = 'food', anim = { dict = 'anim@amb@clubhouse@tutorial@bkr_tut_ig3@', clip = 'machinic_loop_mechandplayer' } },
        { coords = vec3(256.52, -810.88, 30.2), radius = 1.0, label = 'Prepare Kebabs', type = 'food', anim = { dict = 'anim@amb@clubhouse@tutorial@bkr_tut_ig3@', clip = 'machinic_loop_mechandplayer' } },
        -- { coords = vec3(257.21, -815.29, 30.2), radius = 1.0, label = 'Prepare Fries', type = 'food', anim = { dict = 'anim@amb@clubhouse@tutorial@bkr_tut_ig3@', clip = 'machinic_loop_mechandplayer' } },
        { coords = vec3(258.14, -811.52, 30.2), radius = 1.0, label = 'Prepare Drinks', type = 'drink', anim = { dict = 'anim@amb@nightclub@mini@drinking@drinking_shots@ped_a@normal', clip = 'pour_one' } },
    },
    registers = { -- The restaurant registers to charge the customers
        { coords = vec3(256.14, -818.0, 30.2), radius = 0.5, label = 'Register', playersRadius = 5.0, comission = 70 },
    },
    menus = { -- The restaurant menu locs where players can take the menus items (the label must be the same for all)
        { coords = vec3(256.89, -818.37, 30.2), radius = 0.5, label = 'Menu' },
    },
