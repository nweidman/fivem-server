...
createRestaurants = function()
    ...
    for k, v in pairs(Config.restaurants) do
        if v.enabled then
            ...
            lib.zones.poly({ points = v.zone.points, thickness = v.zone.thickness, debug = Config.debug, onEnter = function() onZoneEnter(v.job, k) end, onExit = function() onZoneExit(v.job, k) end })
                    ...
                    local options = {
                        {
                            ...
                            onSelect = function() takeBox(v.job) end,
                            action = function() takeBox(v.job) end,
                            canInteract = function()
                                return (getPlayerJob() == v.job)
                            end,
                        }
            ...
            if v.chairs then
                for a, b in pairs(v.chairs) do
                    local targetId = 'chair_target_' .. k .. '_' .. a

                    exports.ox_target:addSphereZone({
                        name = targetId,
                        coords = b,
                        radius = 0.5,
                        debug = false, -- set to true if you want to visualize the zone
                        options = {
                            {
                                name = targetId,
    ...
    for k, v in pairs (Config.shops) do
        if v.enabled then
            createBlip(vec3(v.ped.coords.x, v.ped.coords.y, v.ped.coords.z), v.blip.sprite, v.blip.display, v.blip.scale, v.blip.color, v.label)
            local ped = createPed(v.ped.coords, v.ped.model, { scenario = v.ped.scenario})
            local options = {
                {
                    distance = 2.5,
            ...
            createEntityTarget(ped, options, 2.5, 'ingredients_shop_'..k)
        end
    end
end
