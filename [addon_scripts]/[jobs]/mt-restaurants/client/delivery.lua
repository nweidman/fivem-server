...
takeOrder = function(job, id)
    ...
    if (not recipes) or (#recipes < 1) then notify(locale('notify_no_recipes_created'), 'error') return end
    for k, v in pairs(recipes) do
        if not (v.active == 1 or v.active == true) then recipes[k] = nil end
    end
    if (not recipes) or (#recipes < 1) then notify(locale('notify_no_recipes_created'), 'error') return end
        ...
        local options = {
            {
                ...
                canInteract = function()
                    if getPlayerJob() == job and inDelivery and (not giving) then return true end return false
                end,
            }
        }
