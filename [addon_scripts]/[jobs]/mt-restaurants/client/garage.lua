...
openGarage = function(job, id)
    ...
    if not spawnedVehicle then
        for k, v in pairs(Config.restaurants[job].garage[id].vehicles) do
            menu.options[#menu.options+1] = { title = v.label, icon = v.icon, arrow = true,
                onSelect = function()
                    spawnedVehicle = spawnVehicle(v.id, Config.restaurants[job].garage[id].spawnCoords)
                end
            }
        end
    else
        menu.options[#menu.options+1] = { title = locale('menu_garage_store_vehicle'), icon = 'ban', arrow = true,
            onSelect = function()
                removeVehicleKeys(spawnedVehicle)
