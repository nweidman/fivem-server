openRegister = function(job, id)
    lib.callback('mt-restaurants:server:makePlayersList', false, function(onlineList)
        ...
        local input = lib.inputDialog(Config.restaurants[job].registers[id].label, {
            ...
            { type = 'select', icon = locale('input_register_payment_type_icon'), required = true, label = locale('input_register_payment_type'), default = 'money', options = { { value = Config.framework == 'esx' and 'money' or 'cash', label = locale('input_register_cash') }, { value = "bank", label = locale('input_register_bank') } } },
