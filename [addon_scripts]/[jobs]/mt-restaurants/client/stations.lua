            ...
            for k, v in pairs(recipes) do
                if tonumber(v.active) == 1 then
                    if haveStation(station, json.decode(v.stations)) then
                        ...
                        for a, b in pairs(json.decode(v.ingredients)) do
                            if a ~= 'no_ingredient' then
                                if not itemData(tostring(a)) then
                                    ...
                                    lib.print.error("The recipe "..v.label.." recipe have some items that are not added to the inventory items.lua!!")
                        ...
                        menu.options[#menu.options+1] = { disabled = disabled, title = locale('menu_prepare_title', v.label), description = neededItems, icon = v.image_url, onSelect = function() prepareItemStation(job, id, Config.restaurants[job].stations[id].label, Config.restaurants[job].stations[id].anim, v.item, { imageurl = v.image_url, label = v.label, description = v.description, calories = calories, restaurant = Config.restaurants[job].label, animation = v.animation, type = v.type, price = v.price, customBuffs = customBuffs }, tempIngredients) end }
                    end
                end
            end
        end
