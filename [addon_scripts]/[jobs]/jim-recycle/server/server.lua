...
RegisterServerEvent("jim-recycle:TradeScrap", function()
    local src = source
	local table = {}

	local amount = exports.ox_inventory:GetItem(src, 'ls_auto_parts', nil, true)

	for i = 1, amount do
		local item = Config.ScrapItems[math.random(1, #Config.ScrapItems)]
		if not table[item] then table[item] = 0 end
		table[item] = table[item] + 1
	end
	if exports.ox_inventory:RemoveItem(src, 'ls_auto_parts', amount) then
		for k,v in pairs(table) do
			TriggerEvent("jim-recycle:server:toggleItem", true, k, v, src)
		end
		exports.qbx_core:Notify(src, "You have exchanged your auto parts!", 'success')
	end
end)
