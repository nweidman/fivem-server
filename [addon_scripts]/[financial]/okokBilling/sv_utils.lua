...
function removeMoney(xPlayer, account, value)
	xPlayer.Functions.RemoveMoney(account, tonumber(value), 'paid PD/EMS bill')
end

function addMoney(xPlayer, account, value)
	xPlayer.Functions.AddMoney(account, tonumber(value), 'billing commission')
end
...
RegisterServerEvent("okokBilling:SocietyBill", function(target_source, source, fine, reason)
	local Player = QBCore.Functions.GetPlayer(source)
	local TPlayer = QBCore.Functions.GetPlayer(target_source)
	data = {
		authorPlayer = Player,
		receiverPlayer = TPlayer,
		item = reason,
		price = fine,
		note = ""
	}

	TriggerEvent('okokBilling:createInvoiceSociety', data)
end)
	...
	if Config.AuthorReceivesAPercentage then
		...
		if xAuthor == nil then
			MySQLfetchAll(playerAccountsQuery()..' = @id', {
				...
				if account ~= nil and json.encode(account) ~= '[]' then
					local playerAccount = getPlayerAccount(account)
					playerAccount = json.encode(setPlusAccount(playerAccount, authorReceive))
						...
						MySQLexecute(playerAccountSetQuery()..' = @playerAccount WHERE citizenid = @target', {
							...
							MySQLexecute('UPDATE '..Config.DatabaseTable..' SET status = "paid", paid_date = CURRENT_TIMESTAMP() WHERE ref_id = @ref_id', {
								...
								if Config.AuthorReceiveNotification then
									...
									local authorReceive = Config.AuthorPercentage / 100
									if xAuthor ~= nil then
										TriggerClientEvent(Config.EventPrefix..':notification', xAuthor.PlayerData.source, "Invoice", "A new invoice was paid", 5000, "success")
										addMoney(xAuthor, "bank", authorReceive * invoice.invoice_value)
									else
										local result = MySQL.Sync.fetchAll('SELECT * FROM players WHERE citizenid = ?', {invoice.author_identifier})
										local RecieverMoney = json.decode(result[1].money)
										RecieverMoney.bank += authorReceive * invoice.invoice_value
										MySQL.Async.execute('UPDATE players SET money = ? WHERE citizenid = ?', {json.encode(RecieverMoney), invoice.author_identifier})
									end
								end
								if(cb ~= nil) then
