			...
			if result[1] ~= nil  then
				if bankMoney >= vehicle_price then
					MySQL.query('UPDATE player_vehicles SET citizenid = @target, license = @license WHERE citizenid = @owner AND plate = @plate', {
						...
						if result2 ~= 0 then	
							tPlayer.Functions.RemoveMoney('bank', vehicle_price, 'vehicle purchase/transfer')
							xPlayer.Functions.AddMoney('bank', vehicle_price, 'vehicle sale/transfer')
...
function sellVehicleWebhook(data)
	local xPlayer = QBCore.Functions.GetPlayer(data.source_id)
	local tPlayer = QBCore.Functions.GetPlayer(data.target_id)
	local information = {
		{
			...
			["description"] = '**BUYER**\n**Name: **'..tPlayer.PlayerData.name..'\n**Character Name: **'..data.target_name..'\n**Citizen ID: **'..tPlayer.PlayerData.citizenid..'\n**License: **'..tPlayer.PlayerData.license..'\n\n**SELLER**\n**Name: **'..xPlayer.PlayerData.name..'\n**Character Name: **'..data.source_name..'\n**Citizen ID: **'..xPlayer.PlayerData.citizenid..'\n**License: **'..xPlayer.PlayerData.license..'\n\n**VEHICLE**\n**Model: **'..data.model..'**\nPlate: **'..data.plate..'**\nPrice: **$'..data.vehicle_price,
