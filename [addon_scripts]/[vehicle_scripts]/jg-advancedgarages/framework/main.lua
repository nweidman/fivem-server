...
Framework = {
  ...
  Queries = {
    ...
    StoreVehicle = "UPDATE %s SET in_garage = 1, garage_id = ?, fuel = ?, body = ?, engine = ?, damage = ? WHERE %s = ? AND REPLACE(UPPER(plate), ' ', '') = ?",
    ...
    ImpoundVehicle = "UPDATE %s SET impound = 1, impound_retrievable = ?, impound_data = ?, garage_id = ?, fuel = ?, body = ?, engine = ?, damage = ?, in_garage = 1 WHERE plate = ?",
