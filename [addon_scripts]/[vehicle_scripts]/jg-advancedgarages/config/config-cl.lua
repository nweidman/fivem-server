...
RegisterCommand('vehadd', function(source, args)
  if #args < 2 then
    Framework.Client.Notify("Usage: /vehadd <model> <targetId>", "error")
    return
  end
  ...
  if IsModelValid(modelHash) then
    TriggerServerEvent('jg-advancedgarages:server:addVehicleToGarage', targetId, model)
    -- Framework.Client.Notify("Giving vehicle (" .. model .. ") to player ID: " .. targetId, "info")
  else
    Framework.Client.Notify("Invalid vehicle model: " .. model, "error")
  end
end, false)

TriggerEvent('chat:addSuggestion', '/vehadd', 'Give a vehicle to a player by model and target ID', {
  { name = 'model', help = 'Vehicle model name (e.g., panto)' },
  { name = 'targetId', help = 'Player ID to give the vehicle to' }
})