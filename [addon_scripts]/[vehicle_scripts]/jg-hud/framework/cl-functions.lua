...
function Framework.Client.ToggleSeatbelt(vehicle, seatbeltOn)
  if GetResourceState("qbx_harness") == "started" and vehicle and DoesEntityExist(vehicle) then
    local hasHarness = Entity(vehicle).state and Entity(vehicle).state.harnessInstalled == true
    ...
    if seatbeltOn then
      exports["qbx_harness"]:enableSeatbelt()
    else
      exports["qbx_harness"]:disableSeatbelt()
    end
