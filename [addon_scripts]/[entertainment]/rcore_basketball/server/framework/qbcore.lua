...
CreateThread(function()
    if Config.Framework == 2 then
        ...
        PlayerGiveMoney = function(source, amount)
            return QBCore.Functions.GetPlayer(source).Functions.AddMoney('cash', amount, 'basketball')
        end

        PlayerTakeMoney = function(source, amount)
            return QBCore.Functions.GetPlayer(source).Functions.RemoveMoney('cash', amount, 'basketball')
        end
        ...
        PlayerTakeItem = function(serverId, itemName)
            ...
            if result then
			    TriggerClientEvent('qb-inventory:client:ItemBox', serverId, QBCore.Shared.Items[itemName], 'remove')
                return true
            end
        end
        ...
        PlayerGiveItem = function(serverId, itemName)
            ...
            TriggerClientEvent('qb-inventory:client:ItemBox', serverId, QBCore.Shared.Items[itemName], 'add')
        end
        
        if Config.OxInventory then
            QBCore.Functions.CreateUseableItem = function(itemName, cb)
                exports(itemName, function(event, item, inventory, slot, data)
                    if event and (event ~= 'usedItem' and event ~= 'usingItem') then
                        return
                    end
                    cb(inventory.id)
                end)
