...
AddEventHandler('rcore_pool:turnGranted', function(tableAddress)
    ...
    if not TableData[tableAddress].balls[TableData[tableAddress].cueBallIdx].disabled then -- cue ball in play
        ...
        local sendPos = TableData[tableAddress].balls[TableData[tableAddress].cueBallIdx].position
        TriggerServerEvent('rcore_pool:setBallInHandData', GetServerIdsNearTable(ClosestTableAddress), tableAddress, {x = sendPos.x, y = sendPos.y})
...
function SetupBallInHand(heading, tableAddress)
    ...
    Citizen.CreateThread(function()
        ...
        while CurrentState == STATE_BALL_IN_HAND do
            ...
            local realPos = GetEntityCoords(cueBall.entity)
            SetEntityCoordsNoOffset(cueBall.entity, cueBall.position.x, cueBall.position.y, realPos.z, false, false, false)
            local nowTime = GetGameTimer()
            
            if (nowTime - lastBallInHandSync) > 500 then
                TriggerServerEvent('rcore_pool:setBallInHandData', GetServerIdsNearTable(ClosestTableAddress), tableAddress, {x = cueBall.position.x, y = cueBall.position.y})
...
RegisterCommand('debuglastpoolshot', function()
    print(json.encode(lastShotData))
end)
...
function SetupAiming(fromCam, tableAddress)
    ...
    Citizen.CreateThread(function()
        while CurrentState == STATE_AIMING do
            ...
            RenderPreviewShotLine(TableData[tableAddress].entity, cueBallPos, offsetPos)

            if not isSettingStrength then
                local step = 60 * GetFrameTime()

                if IsControlPressed(0, Config.Keys.AIM_SLOWER.code) or IsDisabledControlPressed(0, Config.Keys.AIM_SLOWER.code) then
                    step = 60 * GetFrameTime() * 0.13
                end
        ...
        if selectedStrikeStrength then
            ...
            lastShotData = {
                pos = currentTableCueBall.position, 
                vel = selectedStrikeVelocity, 
                str = selectedStrikeStrength,
                balls = TableData[tableAddress].balls
            }

            TriggerServerEvent(
                'rcore_pool:syncCueBallVelocity', 
                GetServerIdsNearTable(tableAddress), 
                tableAddress, 
                {x = currentTableCueBall.position.x, y = currentTableCueBall.position.y}, 
                {x = selectedStrikeVelocity.x, y = selectedStrikeVelocity.y}, 
                selectedStrikeStrength
            )
