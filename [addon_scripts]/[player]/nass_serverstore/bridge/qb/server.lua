...
function addItem(source, item, count)
    local xPlayer = GetPlayer(source)
    TriggerClientEvent('qb-inventory:client:ItemBox', source,  item, 'add')
...
function addMoney(source, type, amount)
    ...
    local xPlayer = GetPlayer(source)
    xPlayer.Functions.AddMoney(type, amount, "Crown Redeem")
end
...
function addVehtoDB(src, props, model, vehType)
    ...
    if GetResourceState('qbx_core') == 'started' then
        ...
        MySQL.insert('INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, garage_id, state, in_garage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', {xPlayer.PlayerData.license,xPlayer.PlayerData.citizenid,model,GetHashKey(model),json.encode(props),props.plate,'Legion Square',1,1}, function()
            SendToDiscord('Vehicle Redeemed', GetPlayerName(src)..' redeemed their car!', 15158332)
        end)
    end
end
...
function GeneratePlate()
    local plate, exists
    repeat
        plate = (QBCore.Shared.RandomStr(4) .. QBCore.Shared.RandomInt(4)):upper()
        exists = MySQL.scalar.await('SELECT 1 FROM player_vehicles WHERE plate = ? LIMIT 1', { plate })
    until not exists
    return plate
end
