		...
		if result[1] then
			...
			for _, i in pairs(boughtPackages) do
				...
				if Config.Packages[i] ~= nil then
					...
					SendToDiscord('Code Redeemed', '**REDEEMED**\n**Product: **'..i..'\n\n**PLAYER**\n**Name:** '..Player.PlayerData.name..'\n**Character Name: **'..xName..'\n**Citizen ID: **'..identifier..'\n**License: **'..Player.PlayerData.license..'', 3066993)
				else
			...
			if result[1] then
				...
				MySQL.update('UPDATE codes SET packagename = ? WHERE code = ?', {json.encode(packagetable), tbxid}, function(rowsChanged)
					if rowsChanged > 0 then
						SendToDiscord('Purchase Completed', '**PURCHASED**\n**Product: **'..dec.packagename..'\n**Tebex Code: **'..tbxid..'', 1752220)
					else
