local config = require('config.chat').chatLogs
...
exportHandler('addMessage', function(target, message)
    if not message then
        message = target
        target = -1
    end
...
exportHandler('registerMessageHook', function(hook)
...
exportHandler('registerMode', function(modeData)
...
AddEventHandler('playerJoining', function()
    if config.joinLeave.status then
        TriggerEvent('um-chat:server:logs:addLogs', source, 'join-leave', '✅ Player joined the server', 'green')
    end
    if not config.joinLeave.firstMessageShowWindow then return end
    TriggerClientEvent('chat:addMessage', -1, {
        args = '',
        title = 'Welcome, ' .. GetPlayerName(source), -- optional
        tag = {
...
AddEventHandler('playerDropped', function(reason)
    if config.joinLeave.status then
        TriggerEvent('um-chat:server:logs:addLogs', source, 'join-leave', '⭕ Player left the server: ' .. reason, 'red')
    end
    if not config.joinLeave.firstMessageShowWindow then return end
    TriggerClientEvent('chat:addMessage', -1, {
        args = reason or 'unkown reason',
        title = 'Good bye, ' .. GetPlayerName(source) .. 'Player Left', -- optional
        tag = {
            name = '😿 Player Left',
            background = '#f87171'
        },
