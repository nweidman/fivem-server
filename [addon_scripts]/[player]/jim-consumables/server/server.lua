	...
	if not give then
		if HasItem(src, item, amount) then -- check if you still have the item
			if QBCore.Functions.GetPlayer(src).Functions.GetItemByName(item).unique then -- If unique item, keep removing until gone
				...
				TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items[item], "remove", amount) -- Show removal item box when all are removed
				return
			end
...
QBCore.Functions.CreateUseableItem("offportspack", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent("consumables:client:offportspack", source, item.name)
end)

QBCore.Functions.CreateUseableItem("offportcigarette", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if QBCore.Functions.HasItem(source, 'lighter', 1) then
        ...
        TriggerClientEvent("consumables:client:use_offportcigarette", src)
    else
        TriggerClientEvent('QBCore:Notify', src, "You need a lighter to smoke!", 'error', 5000)
    end
end)

RegisterNetEvent('consumables:server:packforcigs', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.Functions.RemoveItem('offportspack', 1) then
        Player.Functions.AddItem("offportcigarette", 20)
    end
end)

RegisterNetEvent('consumables:server:usedcig', function()
	local Player = QBCore.Functions.GetPlayer(source)
	Player.Functions.RemoveItem('offportcigarette', 1)
end)