...
RegisterNetEvent('jim-consumables:Consume', function(itemName)
    ...
    LocalPlayer.state:set("inv_busy", true, true) TriggerEvent('inventory:client:busy:status', true) TriggerEvent("qb-inventory:client:closeinv") TriggerEvent('canUseInventoryAndHotbar:toggle', false)
    ...
    if Config.UseProgbar then
        if Config.ProgressBar == "ox" then
            CreateThread(function() if exports.ox_lib:progressBar({ duration = time, label = string..QBCore.Shared.Items[itemName].label.."..", useWhileDead = false, canCancel = true,}) then consuming = false else cancelled = true if DoesEntityExist(attachProp) then destroyProp(attachProp) attachProp = nil end if DoesEntityExist(attachProp2) then destroyProp(attachProp2) attachProp2 = nil end end end)
        else
...
RegisterNetEvent('consumables:client:offportspack', function()
    QBCore.Functions.Progressbar("offports_pack", "Unboxing...", 6000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("consumables:server:packforcigs")
    end)
end)

RegisterNetEvent('consumables:client:use_offportcigarette', function()
    TriggerEvent("qb-inventory:client:ItemBox", QBCore.Shared.Items["offportcigarette"], "remove")
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        QBCore.Functions.Progressbar("smoke_offport", "🚬Lightin up..", 6000, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function() -- Done
            TriggerServerEvent("consumables:server:usedcig")
