...
RegisterNUICallback('buyItem', function(data, cb)
    if hasPurchased then
        cb({ success = false, message = "You can only buy one item!" })
        return
    end
    local carData = nil
    if data.itemInfo.itemType == "vehicle" then
        local vehItemNameHash = GetHashKey(data.itemInfo.itemName)
        RequestModel(vehItemNameHash)  while not HasModelLoaded(vehItemNameHash) do Wait(0) end
        local playerPed = PlayerPedId()
        local coords = GetEntityCoords(playerPed)
        local vehicle = CreateVehicle(vehItemNameHash, coords.x, coords.y, coords.z - 50, 1.0, false, false)
