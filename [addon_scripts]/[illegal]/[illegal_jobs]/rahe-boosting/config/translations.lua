...
-- Make sure this file is included as a shared_script in fxmanifest.lua
...
translations = translations or {
    ...
    NOTIFICATION_TABLET_ORDER_SUCCESSFUL = "Order successful. You can pick up your goods from a storage door near postal 868. The order won't be there for long!",
    ...
    -- these may receive floats/strings; use %s to avoid Lua 5.4 integer errors
    NOTIFICATION_GAME_HACK_SUCCESSFUL = "GPS device transmitting frequency increased by %s seconds. You've completed %s of %s hacks.",
    NOTIFICATION_GAME_NO_GPS_DEVICE = "This vehicle doesn't have a GPS tracking device to hack.",
    NOTIFICATION_GAME_HACK_COOLDOWN = 'The next hack is available in %s seconds.',
    ...
    NOTIFICATION_GAME_SUCCESSFUL_DELIVERY = 'You received $%s and %s %s for the successful delivery.',
    NOTIFICATION_GAME_PENALIZED_DELIVERY = 'You received $%s less than you should have, because the vehicle was damaged.',
...
-- Safe translator:
function T(key, ...)
    local s = translations and translations[key]
    if not s then
        print(('[translations] missing key: %s'):format(tostring(key)))
        return key
    end
    local argc = select('#', ...)
    if argc == 0 then return s end

    -- Coerce args to safe types to avoid %d/number issues
    local args = {...}
    for i = 1, argc do
        if type(args[i]) == 'boolean' then
            args[i] = args[i] and 1 or 0
        end
    end

    local ok, out = pcall(string.format, s, table.unpack(args, 1, argc))
    if ok then return out end
    print(('[translations] format error for key %s: %s'):format(tostring(key), out))
    -- fallback: just return unformatted string + show args
    return s
end
