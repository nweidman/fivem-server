...
local savedAppearance -- stores the player’s pre-gear look

--- @param diving boolean
function ChangeClothes(diving)
    local ped = cache.ped

    if diving then
        -- Save once (so taking gear off restores the exact previous outfit)
        if not savedAppearance then
            savedAppearance = exports['crm-appearance']:crm_get_ped_appearance(ped)
        end

        local isMale = (GetEntityArchetypeName(ped) == 'mp_m_freemode_01')
        ...
        for _, clothes in pairs(shared.gear.clothes) do
            ...
            SetPedComponentVariation(ped, clothes.componentId, drawableId, textureId, 0)
        end
    ...
    if savedAppearance then
        exports['crm-appearance']:crm_set_ped_appearance(ped, savedAppearance)
        savedAppearance = nil
    else
        -- fallback if something called "remove gear" without ever saving
        TriggerEvent("crm-appearance:load-player-skin")
    end

    Wait(1000)
    TriggerEvent('drool_skills:client:refreshBreathing')
end
...
function AddTargetEntity(entity, data)
    if shared.setup.interact == 'ox_target' then
        ...
        if GetEntityModel(entity) == `xm3_prop_xm3_crate_supp_01a` or GetEntityModel(entity) == `sm_prop_smug_crate_s_bones` or GetEntityModel(entity) == `sm_prop_smug_crate_s_tobacco` then
            CreateThread(function()
                ...
                while DoesEntityExist(entity) do
                    ...
                    if dist <= 30 and not glowing then
                        SetEntityDrawOutline(entity, true)
                        SetEntityDrawOutlineColor(140,123,182, 200) -- red glow
                        SetEntityDrawOutlineShader(1)
                        glowing = true
                    elseif dist > 30 and glowing then
...
local function useRefill()
    ...
    local input = lib.inputDialog('Refill Tank', {
        {type = 'slider', label = 'Select how much oxygen to transfer', required = true, min = 0, max = maxTransfer, step = 0.01 },
    })
